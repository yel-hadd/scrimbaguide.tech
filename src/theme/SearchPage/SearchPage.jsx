...
function SearchPageContent() {
    ...
    const filteredResults = useMemo(() => {
      if (!searchResults || !grouped) return searchResults;
      if (category === 'All') return searchResults;
      return grouped[category] || [];
    }, [searchResults, grouped, category]);
    const perGroupCounts = useMemo(() => {
      if (!grouped) return { All: 0, Courses: 0, Blog: 0, Docs: 0 };
      return { All: searchResults?.length || 0, Courses: grouped.Courses?.length || 0, Blog: grouped.Blog?.length || 0, Docs: grouped.Docs?.length || 0 };
    ...
    return (<React.Fragment>
      ...
      <div className="container margin-vert--lg">
        ...
        <section>
          {filteredResults &&
            filteredResults.map((item) => (<SearchResultItem key={item.document.i} searchResult={item}/>))}
        </section>
      </div>
    </React.Fragment>);
}
