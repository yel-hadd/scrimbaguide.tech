...
Six array drills you can run in a browser console or a Scrimba scrim, no setup required, running from a `map`/`filter`/`reduce` warm-up to a two-array join that shows up in junior interviews. Each has starting code, an expected result, and a solution with an explanation. For anyone who can write `array.map(x => x * 2)` but freezes when asked to group or sort real data.
