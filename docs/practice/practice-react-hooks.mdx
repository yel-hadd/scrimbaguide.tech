...
Six drills covering hooks mistakes that show up in real components and interviews: stale closures, cleanup, data fetching, custom hooks, and `useReducer`. Each has starting code and a solution with an explanation, not just a course list. Open a free Scrimba scrim alongside them to run the code instead of tracing it on paper.
