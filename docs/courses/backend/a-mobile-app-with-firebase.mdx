...
sidebar_label: "Build a Mobile App with Firebase"
...
A two-hour Pro project course from Rafid Hoda. You build a shopping list app in plain HTML, CSS and JavaScript, store its items in a Firebase Realtime Database, and end by installing it on your phone's home screen. I read the transcript of every one of its 38 scrims, opened the code in each one, and took the screenshots below with a Pro account in September 2026.

*This page is part of our [Scrimba backend courses](/docs/courses/backend/) catalog. Scrimba's course page does not list it under any career path.*

## Quick answer

Build a Mobile App with Firebase is a short, guided build. There are no modules, just 38 scrims in one long list, most of them two to five minutes. You start with an input field and a button, connect a Firebase Realtime Database in the fourth scrim, and by the halfway point items you type are saved to the cloud and rendered back in real time. The second half adds delete-on-click, mobile styling, a favicon, a web app manifest, and a Netlify deploy. The whole thing is Pro; only the first two scrims are free previews.

It teaches six Firebase functions well (`initializeApp`, `getDatabase`, `ref`, `push`, `onValue`, `remove`) and nothing else about Firebase. No authentication, no Firestore, no hosting, and security rules get a two-minute scrim that ends with "we'll cover this in a future course." If you already write basic JavaScript and want to see data leave the browser for the first time, it is a good two hours. If you want to learn Firebase as a product, it is not that course.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="The only title card in the course: it has no chapters, so this is the opening slide of scrim 1."
...
<Screenshot
  ...
  alt="Scrimba Build a Mobile App with Firebase, the onValue aside paused at 8:03: the editor shows onValue(booksInDB, function(snapshot)) with a for loop calling appendBookToBooksListEl, and the bestreads preview on the right lists Make Time, Animal Farm, Make Timed, Animal Farm, the list duplicated after a database edit"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Build a Mobile App with Firebase, the CSS-for-list-items scrim paused at 3:47: the editor shows the ul and ul li rules with flex-wrap, gap, border-radius and flex-grow, and the preview shows the Add to Cart app with a cat illustration, an input, a red Add to cart button and wrapped item chips reading Oranges, Chocolate, Red bull, Bread, Coffee, Solo, Salad"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Build a Mobile App with Firebase, scrim 21 paused at 0:53: the editor shows the index.js imports from firebasejs 9.15.0 and the push and onValue code, the preview shows the app with one item, Red bull, and the console panel shows a Firebase warning and the error TypeError: Cannot convert undefined or null to object"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Requirements slide from the We are the Champions solo project in Scrimba's Build a Mobile App with Firebase course, listing Build it from scratch, Follow the design, Firebase Realtime Database, Update in realtime, next to a phone mockup of the endorsements app with a Publish button and three endorsement cards"
  width={1279}
  height={720}
