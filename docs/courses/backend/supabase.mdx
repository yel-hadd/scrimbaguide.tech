...
Scrimba's free Supabase course, taught by Jonathan Hill across about 4.8 hours. You connect a React sales dashboard to a real Postgres database, then add sign up, sign in, and permissions. I opened all 49 scrims and read the transcript of every one that has one (46 of them) with a Pro account in September 2026. This is what is inside, not the listing page rewritten.
...
Intro to Supabase is a free, intermediate course in two sections. Persistence (60 minutes, 11 scrims) connects a ready-made React app to a Supabase table, queries it, draws a bar chart, and keeps the chart live with a realtime subscription. Authentication (3.8 hours, 37 scrims) is the real course: routing, a session context, JSON Web Tokens, sign in and sign up, row level security policies, and a database trigger that builds a user profile on sign up. You work in your own Supabase project on its free tier, so what you build is yours at the end.

It suits React developers who have never wired up a backend. It assumes you can write a component, use `useState` and `useEffect`, and have seen a SQL `SELECT`. It is not a Supabase reference: storage, edge functions, OAuth logins, and vector search are mentioned once and never used.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    alt="Title card of the Course introduction scrim in Scrimba's Intro to Supabase course: a white lightning bolt above the words Intro to Supabase and Teacher: Jonathan Hill on a purple and gold gradient"
    width={1377}
    height={774}
    caption="Section 1: the course introduction, which doubles as the Persistence intro."
    source=""
  />
  <Screenshot
    src="/img/scrimba/intro-to-supabase/chapter-2-authentication-title-card.webp"
    alt="Title card of the Introduction to Auth scrim in Scrimba's Intro to Supabase course, reading Section 2 - Authentication, Teacher: Jonathan Hill, on the same purple gradient"
    width={1378}
    height={775}
    caption="Section 2: Authentication, the 3.8-hour half of the course."
...
"Format data for chart" is the one React-heavy scrim, and Jonathan says at 0:16 that it is "more data manipulation and creating the bar chart than pure Supabase, so feel free to skip if you want." It installs `react-charts@beta` and maps your rows into the `{ primary, secondary }` shape the library wants. Then "Realtime subscription" adds `supabase.channel('deal-changes').on('postgres_changes', ...)` inside the same `useEffect`, and you test it by inserting a row in the Supabase table editor and watching the console. He is honest that the solution just refetches everything on any change: "if you had a really large dataset, this would not be very performant."

<Screenshot
  src="/img/scrimba/intro-to-supabase/realtime-subscription-challenge-lesson.webp"
  alt="Scrimba Intro to Supabase lesson Realtime subscription paused at the challenge: the editor shows a five-step challenge brief in a comment and a supabase.channel('').on('postgres_changes', { event: '', schema: '', table: '' }) skeleton with blanks to fill, and the preview on the right shows the Sales Team Dashboard bar chart with bars for Andy, Dwight and Jim"
  width={1400}
  height={660}
  caption="The realtime challenge at 2:56. The brief is the numbered comment at the top, the blanks in the channel() call are yours to fill, and the bar chart from the previous scrim is already running in the preview. This is the shape of most challenges in the course."
/>
...
<Screenshot
  ...
  alt="Slide from the Trigger lesson in Scrimba's Intro to Supabase course showing an entity relationship diagram: an orange users table with id, email, role and user_metadata, a green user_profiles table with id, name and account_type, and a green sales_deals table with id, created_at, user_id and value, connected by foreign key lines"
  width={1400}
  height={788}
  caption="The schema the refactor builds, shown again at 9:08 of the Trigger scrim. user_profiles.id points at auth.users.id, and sales_deals.user_id points at user_profiles.id, which is what lets a policy ask who owns a row."
/>
...
<Screenshot
  ...
  alt="Scrimba Intro to Supabase Trigger lesson paused at the challenge: trigger.sql in the editor shows create function with the name blanked out, returns trigger, language plpgsql, security definer, an insert into with the table and columns left as a comment to complete, and new.raw_user_meta_data ->> 'name'; the preview on the right shows the Paper Like A Boss sign-up form with Admin and Sales Rep radio buttons"
  width={1400}
  height={660}
  caption="The trigger challenge at 8:06. Jonathan has removed the function name, the target table and columns, and the account_type extraction; the sign-up form in the preview already has the Admin and Sales Rep radio buttons from the previous scrim."
/>
...
<Screenshot
  ...
  alt="Scrimba Intro to Supabase lesson Refactor deals table part 2 at 7:20: policy.sql in the editor shows an EXISTS example on an extended_family table, then CREATE policy 'Reps can only add their own deals' ON public.sales_deals FOR insert TO authenticated WITH CHECK (auth.uid() = user_id AND EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.id = auth.uid() AND user_profiles.account_type = 'rep'))"
  width={1400}
  height={660}
  caption="The finished rep policy at 7:20, written out in a scratch policy.sql file before you copy it into the Supabase policy editor. The extended_family example above it is how Jonathan explains EXISTS and SELECT 1 first."
/>
