...
A 46 minute, free course on regular expressions in JavaScript, recorded by Beau Carnes of freeCodeCamp and hosted on Scrimba's player. I read the transcript of every one of its 36 scrims with a Pro account in September 2026, so this page describes what is in each of them, not the listing rewritten.
...
Learn Regular Expressions is freeCodeCamp's regex curriculum re-recorded as 34 Scrimba screencasts, most of them under a minute and a half. It is free, rated Intermediate, and has no modules: one flat list of scrims that starts with `.test()` and ends with capture groups and `.replace()`. There is no project to build and nothing behind a Pro wall. If you already write some JavaScript and want to stop copying regex patterns off the internet without understanding them, it is a good hour. If you want a course that makes you build something, this is not it.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="The only title card in the course, from the intro scrim. The green is freeCodeCamp's, not Scrimba's."
...
<Screenshot
  ...
  alt="Scrimba Learn Regular Expressions lesson Find Characters with Lazy Matching, paused at 3:14: the editor shows let myRegex = /<.*?>/ applied to an h1 string, and the console below prints an array containing only the opening h1 tag"
  width={1400}
  height={625}
  caption="Lazy matching at 3:14. The question mark has just been added to the pattern and the console now shows only the opening tag; two seconds earlier, without it, the same line printed the entire h1 element."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Regular Expressions lesson Restrict Possible Usernames, paused at 2:41: a comment block lists the three username rules, and below it the finished pattern userCheck = /^[A-Za-z]{2,}\d*$/ is highlighted in the editor"
  width={1400}
  height={625}
  caption="The username validator at 2:41, complete. The three rules sit in the comment at the top and the finished pattern is selected on line 8. This is the scrim to pause and rewrite yourself with a different set of rules."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Regular Expressions lesson Reuse Patterns Using Capture Groups, paused at 4:59: the editor shows repeatRegex = /(\w+)\s\1/ with comments noting it returns true and the array regex regex, regex, and below it the finished reRegex = /^(\d+)\s\1\s\1$/ for matching a number repeated three times"
  width={1400}
  height={625}
...
<FAQAccordion items={[
  ...
  { q: "Why does the same scrim title appear twice?", a: "Two consecutive scrims are both called Match Characters that Occur One or More Times. The first one actually teaches negated character sets with the caret; the second teaches the + quantifier. It is a labeling slip, not a duplicate lesson." },
]} />
