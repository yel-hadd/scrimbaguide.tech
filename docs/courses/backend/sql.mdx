...
Scrimba's free SQL course, taught by Gregor Thomson across about 3.8 hours, in which you run a vintage car dealership's database and answer its owner's questions with queries. I opened all 49 scrims and read the transcript of every one of them with a Pro account in September 2026. This page describes what is in the course, lesson by lesson, and who it suits.
...
Learn SQL is a free, beginner-level course in three modules: query a single table, then create and join several tables, then write subqueries and CASE expressions. Every lesson works on the same story: you have been hired by Rodney, CEO of a dealership called Retro Rides, to manage his stock of classic cars. The database runs inside the browser (PGlite, a Postgres build that runs in JavaScript), so there is nothing to install.

It is short, well paced, and heavy on challenges: I counted 62 across 49 scrims. It assumes no prior code, though it moves faster if you have written a little JavaScript. If you want a first, practical grounding in SQL before touching a real backend, this is the course to take. If you already write joins comfortably, it will feel slow until module 3.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={712}
    caption="Module 1 opens with the course title card."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={712}
    caption="Module 2: creating and joining tables."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={712}
    caption="Module 3: ANY, ALL, EXISTS and CASE."
...
The honest limit: this is the query language only. Designing a schema, indexing, transactions, performance, and connecting a database to a real app are out of scope. Gregor says so in lesson two: "setting up tables and building a database could be a whole course in itself."
...
<Screenshot
  ...
  alt="Scrimba Learn SQL lesson OR paused at 2:42: the editor shows SELECT brand, model, condition, price FROM cars WHERE price < 250000 OR brand = 'Porsche' AND condition > 3, and the Runner below lists cars with condition 1, 2 and 3 that should have been excluded"
  width={1400}
  height={660}
  caption="The OR lesson at 2:42. The query has AND condition > 3 on the end, but the results still include cars in condition 1, 2 and 3. Gregor fixes it with brackets around the OR a few seconds later."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn SQL lesson Alter table paused at 6:15: the editor shows the ALTER TABLE statements adding a NOT NULL and a foreign key constraint followed by an INSERT with NULL as the dealership_id, and the Runner shows the error null value in column dealership_id of relation cars violates not-null constraint"
  width={1400}
  height={660}
  caption="Alter table at 6:15. The INSERT at the bottom of the editor passes NULL for dealership_id on purpose, and the Runner prints the not-null constraint error from PGlite. The foreign key violation is demonstrated the same way a minute later."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn SQL lesson Full join, inner join and drop paused at 5:35: the editor shows SELECT name, role, city, state FROM staff FULL JOIN dealerships ON dealership_id = dealerships.id, and the Runner shows staff rows with null city and state followed by dealership rows with null name and role"
  width={1400}
  height={660}
  caption="The full join at 5:35, scrolled to the bottom of the output: three new hires with no dealership yet (null city and state), then six dealerships with no staff (null name and role). Switching FULL to INNER removes all nine rows."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn SQL lesson CASE in SELECT paused at 3:21: the editor shows a CASE expression with WHEN condition >= 4 THEN 'Excellent', WHEN condition >= 3 THEN 'Fair', WHEN condition >= 1 THEN 'Poor', ELSE 'Wrecked', and the Runner lists cars with their numeric condition next to the matching label"
  width={1400}
  height={660}
  caption="CASE in SELECT at 3:21. The CASE block in the editor maps the numeric condition column to a word, and the Runner shows both side by side. The follow-up challenge uses the same shape to compute bonuses per staff member."
/>
