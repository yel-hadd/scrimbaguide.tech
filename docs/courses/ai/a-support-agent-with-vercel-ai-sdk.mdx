...
sidebar_label: "Support Agent with Vercel AI SDK"
...
Scrimba's Pro project course on shipping a customer support agent with the Vercel AI SDK, taught by Mayo Oshin in just under two hours. I went through it with a Pro account in September 2026 and read the transcript of every one of the 23 scrims, from the free embeddings preview to the certificate reminder at the end. Where I quote Mayo, the words come from those transcripts.
...
Build a Support Agent with Vercel AI SDK is a short, intermediate, Pro-tier course: 114 minutes of video by Scrimba's count, across 23 scrims, six of them challenges. You embed Scrimba's own help center articles into a Supabase vector store and learn four functions from the Vercel AI SDK: `generateText`, `embed`, `generateObject`, and `tool`. You finish with an Express-served chatbot that decides, per question, whether to search your docs, search the web, or answer directly.

The first six scrims are free previews; the rest need Pro. It assumes you already know what RAG (retrieval augmented generation, where the model answers from documents you fetch for it) and function calling are, and Mayo says so in the intro. If you have done [Learn AI Agents](/docs/courses/ai/ai-agents/) and want one compact, realistic build on a mainstream SDK, this is a good two-evening project.
