...
Scrimba's free, 84-minute course on building with Mistral's models in JavaScript, made together with Mistral and taught mainly by Per Borgen. I opened every one of its 28 entries and read the transcript of all 25 teaching scrims with a Pro account in September 2026. It is short, but it takes you through three real things: chat completions, a retrieval bot over an employee handbook, and a small agent that calls your functions.
...
Intro to Mistral AI is a free, intermediate course: 84 minutes of video across 25 teaching scrims, plus three generic Scrimba pages at the end. Per Borgen teaches most of it, Gil Hernandez covers the API basics, embeddings and Supabase setup, and Sophia Yang from Mistral opens with a two-minute company intro. You start with chat requests in the Mistral JavaScript client, add streaming and JSON output, and compare the models. Then you chunk a text file with LangChain, embed the chunks with `mistral-embed`, store them in Supabase, and answer questions from them. After that you build an agent loop with function calling, and the last scrim runs Mistral on your own machine with Ollama.

What slows people down is the setup rather than the price. You need a Mistral La Plateforme account with a card on file and a free Supabase project before the RAG section works. The code also pins version 0.1 of Mistral's client, recorded in early 2024, so expect the SDK calls to look different from today's docs.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1278}
    height={718}
    caption="The welcome scrim's title card. Per, Gil Hernandez and Mistral's Sophia Yang share the teaching."
    source=""
  />
  <Screenshot
    ...
    width={1278}
    height={718}
    caption="The function calling section: six scrims that end in a working agent loop."
    source=""
  />
  <Screenshot
    ...
    width={1278}
    height={718}
    caption="The Ollama scrim, where the course leaves the browser and runs on your laptop."
  />
</div>
...
<Screenshot
  ...
  alt="Scrimba Intro to Mistral AI embeddings challenge paused at 0:49: index.js shows a comment block asking you to write createEmbeddings(chunks) and return an array of objects with content and embedding keys, with mistralai 0.1 and langchain 0.1.28 listed as dependencies"
  width={1400}
  height={635}
...
<Screenshot
  ...
  alt="Scrimba Intro to Mistral AI RAG lesson paused at 3:57: index.js shows the four-step flow of input, createEmbedding, retrieveMatches and generateChatResponse, and the console prints a full-sentence answer saying a full-time employee still receives one paid day off when Christmas falls on a Sunday"
  width={1400}
  height={635}
...
<Screenshot
  ...
  alt="Scrimba Intro to Mistral AI function calling lesson paused at 3:11: output.js shows the model's response object with model mistral-large-latest, an empty content string, a tool_calls array naming getPaymentStatus with the argument transactionId T1001, and finish_reason tool_calls"
  width={1400}
  height={635}
