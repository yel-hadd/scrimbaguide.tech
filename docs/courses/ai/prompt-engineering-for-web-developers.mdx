...
Scrimba's Pro course on getting useful work out of ChatGPT as a web developer, taught by Treasure Porth across about 3.1 hours. I opened all 50 scrims and read the transcript of every one with a Pro account in September 2026. This page describes what is actually in the course, including the parts that show their age.
...
Prompt Engineering for Web Developers is a short, Pro-only course about talking to ChatGPT well: about 3.1 hours across three sections, 48 lessons plus two short Scrimba housekeeping scrims, taught by Treasure Porth. You do not build an app. You watch Treasure write prompts, copy the code ChatGPT gives her into the scrim to check that it runs, and then pause to do the same in your own ChatGPT tab. Eleven scrims are flagged as challenges. You do not install anything or buy an API key; a free ChatGPT account is all it needs. It is the most practical prompting course on Scrimba's AI track, and also the one that has aged the most. It was recorded in mid 2023 with ChatGPT and Google Bard, so some tool-specific detail is out of date even though the technique still holds.
...
Two things you should know before paying for it. First, it is a 2023 course. Treasure says on camera that she is recording in June 2023, ChatGPT's training data "stops in late twenty twenty one," and one scrim is built around Google Bard, which no longer exists under that name. The prompting habits transfer to any current model, but expect to translate the tooling. Second, it is not a course about building AI into apps. There is no API call, no SDK, no model provider. For that you want [Intro to AI Engineering](/docs/courses/ai/ai-engineering/).
...
The first five scrims of section one are free previews: the welcome, the intro, Basics Part 1 and 2, and the quiz. Everything after that, 43 of the 48 lessons, needs Pro, as do the certificate and the course's place in the AI Engineer Path. There are no Solo Projects in this course, so there is nothing else to unlock inside it.
