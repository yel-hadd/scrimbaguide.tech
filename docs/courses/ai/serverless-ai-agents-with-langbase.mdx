...
This fits you if you already know what retrieval-augmented generation (RAG) is and want to see the full loop, chunking, embedding, retrieval, and generation, working end to end without setting up a vector database. The catch is that every concept runs through a Langbase method, so if Langbase is not part of your plans, take the transferable mental model to [Learn RAG](/docs/courses/ai/rag/) instead.
...
<CourseSchema
  ...
  instructorUrl="https://scrimba.com/@MahamDev"
  keywords={["AI"]}
  imageUrl="https://scrimbaguide.tech/img/scrimba/build-serverless-ai-agents-with-langbase/chapter-1-building-serverless-ai-agents-title-card.webp"
/>
