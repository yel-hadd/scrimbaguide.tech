...
Scrimba's short Pro course on MCP, the open standard that lets an AI model call tools and read data your app exposes. Maham Codes teaches it in about 37 minutes. I opened all 17 scrims and read the transcript of every one with a Pro account in September 2026; this is what is inside.

*This page is part of our [Scrimba AI courses](/docs/courses/ai/) catalog. Scrimba's course page does not list it under any career path; it is a standalone course.*

## Quick answer

Intro to Model Context Protocol (MCP) is a 37 minute, intermediate, Pro course. You learn what MCP is, then build a small "weather" MCP server in Node.js and TypeScript. It has one tool that returns mock weather for New York or London, one resource that lists those cities, and a transport that runs the server from the terminal. Three of the scrims are challenges where you write the code yourself. The course then shows, on slides, how to test the server with MCP Inspector and plug it into Claude Desktop.

If you already build AI apps in JavaScript and keep hearing "MCP," this is the fastest way to understand it and have a working server by the end of an afternoon. If you have never called an AI model from code, the course will feel abstract; start with [Intro to AI Engineering](/docs/courses/ai/ai-engineering/) instead.
