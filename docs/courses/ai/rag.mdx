...
Scrimba's Pro course on Retrieval-Augmented Generation, or RAG: the technique of fetching relevant text from your own data and handing it to a language model before it answers. Guil Hernandez teaches it in an hour and a half of scrims, from "what is an embedding" to a movie-recommendation chatbot on a Supabase vector database. I read the transcript of every scrim, all 22 of them, with a Pro account in September 2026, and opened the code in each one. This is what is in there, including what has aged.
...
Learn RAG is a short, intermediate, Pro-only course: 94 minutes across 22 scrims in one flat list. You turn text into embeddings with OpenAI, store them in Supabase with the pgvector extension, run similarity searches with a SQL function pasted from the docs, and split a file into chunks with LangChain. It ends with "ReelRecs," a small chat UI that recommends films from your own data.

Three scrims are AI-checked challenges, and the last is a Pro solo project called PopChoice. By the end you can build a "chat with my data" bot for any fixed body of text: product docs, a support knowledge base, a catalog. If you already call the OpenAI chat API from JavaScript, this is the most direct route to the retrieval half of that job. If you have never called a language model from code, start with [Intro to AI Engineering](/docs/courses/ai/ai-engineering/).
...
<div className="screenshot-grid">
  <Screenshot
    ...
    alt="Slide from the What are embeddings? lesson in Scrimba's Learn RAG course: a 2D graph with points for cat (4.5, 12.2), feline (4.7, 12.6) and kitten (5.1, 12.1) clustered together, and dog (7.5, 10.5) and man's best friend (7.6, 10.7) plotted nearby"
    width={1277}
    height={719}
  ...
  <Screenshot
    ...
    alt="Core Requirements slide from the PopChoice solo project in Scrimba's Learn RAG course: build it from scratch, follow the design, use a vector database and the OpenAI API, use the movies array, next to a phone mockup of the PopChoice app with three questions and a Let's Go button"
    width={1277}
    height={720}
...
The honest caveat is age. The scrims pin `openai@4.11.1` and `langchain@0.0.167`, and the models are `text-embedding-ada-002` and `gpt-4`. A 44-second scrim after "Vector databases" has a Scrimba staffer "popping in from the future" because the Supabase package "has been superseded and deprecated." The fix is a right-click and "upgrade package." You will, he warns, need to do that in each scrim. The RAG pattern itself is unchanged.
...
<Screenshot
  ...
  alt="Scrimba Learn RAG challenge Pair text with embedding paused at 2:49: the editor shows the challenge brief as a comment and Guil's solution mapping over the input array with an async callback that calls openai.embeddings.create with model text-embedding-ada-002 and builds a data object with content and embedding, while the console below prints objects whose embedding arrays start with numbers like 0.002089147"
  width={1400}
  height={639}
...
<Screenshot
  ...
  alt="Scrimba Learn RAG lesson Chunking text from documents paused at 5:53: index.js imports CharacterTextSplitter from langchain/text_splitter, fetches podcasts.txt, and constructs the splitter with separator space, chunkSize 150 and chunkOverlap 15 before calling splitter.createDocuments; the sidebar lists langchain@0.0.167 as the only dependency"
  width={1400}
  height={384}
...
<Screenshot
  ...
  alt="Scrimba Learn RAG lesson AI chatbot proof of concept paused at 4:17, with the preview pane in focus over a blurred editor: the ReelRecs app shows a How can I help? input with a send button and a reply beginning Hi Guil! Given your interest in action movies, I would recommend Expend4bles, followed by Top Gun: Maverick and Blue Beetle"
  width={1400}
  height={617}
...
Where it is limited: the packages and models are from late 2023 and need translating to current names. It covers one vector store and one embedding provider, and it stops at a proof of concept with in-memory chat history.
