...
Learn RAG is a short, intermediate, Pro-only course: 94 minutes across 22 scrims in one flat list, turning text into OpenAI embeddings, storing them in Supabase with the pgvector extension, and running similarity searches with a SQL function pasted from the docs before it ends with "ReelRecs," a movie-recommendation chat UI. Three scrims are AI-checked challenges, and the last is a Pro solo project called PopChoice. If you already call the OpenAI chat API from JavaScript this is the most direct route to the retrieval half of that job; if you have never sent a chat completion from code, start with [Intro to AI Engineering](/docs/courses/ai/ai-engineering/) first.
