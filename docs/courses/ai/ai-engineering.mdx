...
Scrimba's core Pro course on building software around large language models, taught by Arsala Khan in about 2.5 hours. I opened every scrim and worked through a sample of them with a Pro account in September 2026. It is a build-along, not a lecture series: you build one app, Gift Genie, from a first API call to a streaming, tool-using, backend-secured product.

*This page is part of our [Scrimba AI courses](/docs/courses/ai/) catalog. It is sequenced into the [AI Engineer Path](/docs/paths/ai-engineer-path/).*

## Quick answer

Intro to AI Engineering is Scrimba's intermediate, Pro-tier course on integrating LLMs into real apps: roughly 2.5 hours across 36 scrims, taught by Arsala Khan. You build Gift Genie, a gift-suggestion app, and along the way learn the messages array, system prompts, token limits, error handling, safe markdown rendering, streaming, few-shot prompting, temperature and top-p, JSON and structured outputs, a web search tool, and finally moving the API key to a Node backend. It uses the OpenAI JavaScript library but is deliberately provider-agnostic: Arsala recommends OpenAI as the default and Groq or OpenRouter as free alternatives. The first five scrims are free samples. If you can already write JavaScript and want to ship an AI feature that works outside a demo, this is the foundation course on the AI track.
...
<CourseCurriculum
  title="Course structure (36 scrims, grouped by topic)"
...
<Screenshot
  ...
  alt="Scrimba Intro to AI Engineering Backend Migration Super Challenge: the file tree shows challenge.md, server.js, index.js, a hints folder and vite.config.js, index.js imports marked and DOMPurify, the Runner panel shows npm run start with a server on localhost:3001 and Vite on 5173, and the preview shows the Gift Genie app with a Rub the Lamp button"
  width={1400}
  height={660}
...
<AffiliateLink href="https://scrimba.com/intro-to-ai-engineering-c032" variant="button">
  Preview Intro to AI Engineering on Scrimba
</AffiliateLink>
