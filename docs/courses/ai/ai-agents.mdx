...
description: "Bob Ziroll's Learn AI Agents reviewed from every transcript: 117 minutes, 31 scrims, a ReAct loop you build by hand, then OpenAI function calling, and what has aged."
...
Scrimba's Pro course on LLM agents, taught by Bob Ziroll in 31 scrims and just under two hours. I read the transcript of every scrim, all 31, with a Pro account in September 2026, and checked the code and dependency panel in a sample of them. This page is what is in there, including the parts that have dated.
...
Learn AI Agents is a short, intermediate, Pro-only course in which you build the same weather-and-location agent twice. An agent, in Bob's definition from the first scrim, is "a computer program or system that's designed to perceive its environment, make decisions, and take actions to achieve a specific goal." First you build one with a ReAct prompt, a system prompt that makes the model write "Thought, Action, PAUSE, Observation" lines, plus a for loop that parses those lines and calls the matching JavaScript function. Then you throw that away and rebuild it with OpenAI's function calling, and finally with a beta SDK helper that runs the loop for you.

There are no modules, just a flat list of 31 scrims, six of them marked as challenges. The first five are free previews. If you want to understand what an agent loop does under the hood, in JavaScript, I have not seen it explained more directly anywhere else on Scrimba. If you want a current framework or a current model, it is not that.

By the end you will be able to write a tool-calling loop from nothing, read an OpenAI `tool_calls` response, and explain why every agent framework has a "max iterations" setting. You will not have a product; the weather tool stays hardcoded the whole way through.
