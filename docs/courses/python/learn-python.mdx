...
description: "Treasure Porth's free Learn Python course reviewed from every transcript: 4.1 hours, three terminal apps, a challenge in nearly every scrim, and what it leaves out."
...
Scrimba's free beginner Python course, rebuilt in August 2026 and taught by Treasure Porth across about 4.1 hours and three small terminal apps. I read the transcript of every scrim, all 61 of them, with a Pro account in September 2026. This page covers what is in there, what each challenge asks of you, and what the rebuild dropped.
...
Learn Python is Scrimba's free, beginner Python course: 4.1 hours across three modules, taught by Treasure Porth. Each module is a build. You write PayUp, an expense splitter that takes four inputs and prints who owes what. Then Jargon Jumble, a five-round word scramble game with hints, skips and a score. Then Sing Out, a karaoke queue manager whose commands are functions you write and which has to survive bad input.

Almost every scrim after the intros carries a challenge icon, so you type more than you watch. No prior programming is assumed, and the whole course is free. It stops at functions and try/except: no dictionaries, no classes, no files. If you want a gentle first language and a finished program every hour or so, it is a good weekend. If you need Python for data work or Django, it is a starting point, not the course.
...
<div className="screenshot-grid">
  ...
  <Screenshot
    ...
    alt="Scrimba Learn Python lesson What We'll Build: the editor shows payup.py with input() calls wrapped in float() and int(), and the terminal below shows the finished PayUp run for Paul's Pizza with cost 345, a 20 percent service charge, group size 8, grand total 414.00 and each person must PayUp 51.75"
    width={1400}
    height={625}
...
<Screenshot
  ...
  alt="Scrimba Learn Python lesson PayUp App: Calculate the Expense Split, paused during the solution: the editor shows the three-step challenge brief in comments above service_charge_total = cost * service_charge / 100, and the terminal shows a run for Karaoke at the Waypost with Service charges: $15, Grand total: $230.0 and Each person must PayUp: $76.66666666666667"
  width={1400}
  height={625}
...
<Screenshot
  ...
  alt="Scrimba Learn Python module 2 intro showing the finished Jargon Jumble game in the terminal: round 1 answered deadline and marked Correct, round 2 answered zebra and marked Sorry the word was bandwidth, round 3 skipped, round 4 quit, then Thanks for playing, Final score: 1/5 and the line Have you tried turning it off and on again?"
  width={1400}
  height={625}
...
<Screenshot
  ...
  alt="Scrimba Learn Python lesson Project: Add Game Loop at 1:44: the editor shows ROUNDS = 5, round_num = 1 and a while round_num <= ROUNDS loop around the game code, and the terminal shows a correct guess of server followed by Round 1 printing a second time instead of Round 2"
  width={1400}
  height={625}
...
<Screenshot
  ...
  alt="Scrimba Learn Python module 3 intro showing the finished Sing Out karaoke queue app in the terminal: a numbered queue of four singers and songs, the next command announcing NOW UP: Mike, I Will Survive, then the top command answered with the word apple and the app replying Please enter a number and reprinting the queue"
  width={1400}
  height={612}
