...
<Screenshot
  src="/img/scrimba/learn-firebase/onsnapshot-todate-null-error-lesson.webp"
  alt="Scrimba's Fetching data in realtime with onSnapshot scrim paused at 5:24: index.js shows onAuthStateChanged calling fetchInRealtimeAndRenderPostsFromDB, and the console reads TypeError: Cannot read properties of null (reading 'toDate') above Document written with ID"
  width={1400}
  height={684}
  caption="onSnapshot at 5:24. The post was written (the ID line proves it) and the listener fired before the server timestamp existed, so toDate is called on null. Rafid invites you to pause and work it out before he does."
/>
...
<Screenshot
  src="/img/scrimba/learn-firebase/security-rules-missing-or-insufficient-permissions-lesson.webp"
  alt="Scrimba's Only let authenticated users read and write scrim paused at 4:07: rules.txt holds a Firestore rule set with allow read, write: if false, and the console shows FirebaseError code permission-denied, Missing or insufficient permissions"
  width={1400}
  height={684}
  caption="The first security rules scrim at 4:07. With every rule set to false the app cannot even read its own posts, which is the point: you lock the door first, then let signed-in users back in."
/>
