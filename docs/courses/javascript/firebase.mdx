...
<Screenshot
  ...
  width={1400}
  height={684}
  caption="onSnapshot at 5:24. The post was written (the ID line proves it) and the listener fired before the server timestamp existed, so toDate is called on null."
/>
...
<Screenshot
  ...
  width={1400}
  height={684}
  caption="At 4:07 of the first security rules scrim, every rule is set to false, so the app cannot even read its own posts. That is the point: you lock the door first, then let signed-in users back in."
/>
...
Scrimba's catalog data does not put Learn Firebase on any career path. It still suits a frontend developer on the [Frontend Developer Path](/docs/paths/frontend-developer-path/) or [Fullstack Developer Path](/docs/paths/fullstack-developer-path/) who wants the first taste of storing data somewhere other than localStorage and deciding who is allowed to read it. If you later want the backend that Firebase hides, [Learn Node.js](/docs/courses/javascript/nodejs/) and [Learn Express.js](/docs/courses/javascript/expressjs/) are the complementary route.
