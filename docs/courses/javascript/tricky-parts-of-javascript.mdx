...
A one-hour Pro course from Zack Wilson (PortEXE) on the parts of JavaScript that a first course skips: how scope and hoisting really behave, the three ways to write a function, and promises through to async/await. I read the transcript of every scrim and captured the moments below with a Pro account in September 2026, so this page describes what is actually in the course rather than what its title suggests.
...
The Tricky Parts of JavaScript is an intermediate, Pro-tier course: 58 minutes of video (listed as 59 on Scrimba) across 18 short scrims, four of them challenges. It has three topics, in this order: scope and hoisting (`var` versus `let`, block scope, the global object, the temporal dead zone), functions (declarations, expressions, IIFEs, arrow functions), and asynchronous code (the Promise constructor, `fetch`, `Promise.all`, `async`/`await`). There is no project. The first six scrims, the whole scope block, are free previews; the rest need Pro.

It does not cover closures, `this`, or type coercion. Closures and `this` are mentioned as "coming later" and then never arrive, because the course ends after promises. If you want the explanation behind `undefined` versus `ReferenceError`, or you have been writing `.then()` chains without knowing what a promise is, this is a tight hour. If you want the classic interview trio of closures, `this` and coercion, look elsewhere.
