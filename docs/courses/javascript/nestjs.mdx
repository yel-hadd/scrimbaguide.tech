...
<Screenshot
  ...
  width={1400}
  height={630}
  caption="At 1:20 of Modules & Decorators, the module file above already imports the controller: generating both with the CLI wired that import in for you."
/>
...
<Screenshot
  ...
  width={1400}
  height={630}
  caption="Controller (POST) at 4:11. Don resets the file to this state before handing it to you, then solves the challenge on camera after 4:33."
/>
...
<Screenshot
  ...
  width={1400}
  height={630}
  caption="Service (Create Profile) at 8:29, after both bugs are fixed. The network tab shows the new profile, Kai, in the GET /profiles response, proof the create() method works end to end."
/>
...
<Screenshot
  ...
  width={1400}
  height={630}
  caption="At 6:05 of Exception Filters, the controller's findOne has no try/catch, yet the NotFoundException thrown inside the service bubbles up on its own, the behavior this lesson is named for."
/>
...
All 22 scrims, all 12 challenges, and the project are free. There are no Solo Project (PRO) items in this course; the challenges are inline and open to everyone. What Pro adds is the Certificate of Completion at the end of the list, the [Backend Developer Path](/docs/paths/backend-developer-path/) this course belongs to, and the Pro-only channels on Scrimba's Discord. Basic Discord access is listed as free on Scrimba's pricing page, so the server itself is not gated. See <AffiliateLink href="https://scrimba.com/our-pricing">current plans</AffiliateLink> for what Pro costs in your region.
...
Intro to NestJS sits at the structured end of Scrimba's backend options, after [Learn Node.js](/docs/courses/javascript/nodejs/) and [Learn Express.js](/docs/courses/javascript/expressjs/). Scrimba's teacher card credits Don with contributing to the [Backend Developer Path](/docs/paths/backend-developer-path/), where this course lives; it is not part of the [Frontend Developer Path](/docs/paths/frontend-developer-path/) or the [Fullstack Developer Path](/docs/paths/fullstack-developer-path/). The natural pair for it is [Learn TypeScript](/docs/courses/typescript/learn-typescript/), since Nest leans on TypeScript classes and decorators throughout.
