...
Scrimba's free, 83 minute introduction to NestJS is taught by DonTheDeveloper (Don Hansen on the title card). You build the profile feature of a dating app for developers. I read the transcript of 21 of the 22 scrims with a Pro account in September 2026 and worked the challenges in a handful of them. The one scrim whose transcript would not load, Pipes (Validation), I opened and describe from its finished code and network tab. This page describes what is in the course, not what the listing says.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course page lists it under the [Backend Developer Path](/docs/paths/backend-developer-path/), and our course data also places it in the [Frontend Developer Path](/docs/paths/frontend-developer-path/) and the [Fullstack Developer Path](/docs/paths/fullstack-developer-path/).*

## Quick answer

Intro to NestJS is a free, intermediate course of 22 scrims and about 83 minutes of video. It is one flat list of lessons, not a set of modules. You scaffold a real NestJS 11 project with the Nest CLI (the command line tool that generates files for you) inside Scrimba's browser IDE, which has a terminal, a runner, a console, and a network request tab.

The project is the `profiles` feature of DevMatch. By the end you have a module, a controller with GET, POST, PUT, and DELETE routes, a service that owns an in-memory array, DTOs, exception handling, transformation and validation pipes, and a guard. Twelve of the 22 scrims carry Scrimba's "Challenge with Instant Feedback" icon, meaning they stop and check your solution.

It assumes you already know Node and have built something with Express. If that is you and you want to see what an opinionated, TypeScript-first framework looks like in an afternoon, it is a good use of 83 minutes. It does not touch databases, authentication, or tests.
