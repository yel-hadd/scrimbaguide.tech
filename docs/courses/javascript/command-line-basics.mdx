...
Scrimba's free introduction to the terminal, taught by Ajo Borgvold in 101 minutes across two short sections. I opened every module and read the transcript of every scrim with a Pro account in September 2026, and the most useful thing I can tell you is that the two halves of this course are built differently: the first half is hands-on in Scrimba's terminal, the second half is slides you follow along with on your own machine.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. The course page shows no path badge, but the module list of Scrimba's [Backend Developer Path](/docs/paths/backend-developer-path/) includes a 103 minute "Command Line Interface" module that matches this course.*

## Quick answer

Command Line Basics is a free, beginner course of 24 short scrims, none longer than eight minutes. Section one puts a real terminal inside the Scrimba editor and walks you through `pwd`, `ls`, `cd`, `touch`, `rm`, `mkdir`, `echo` and `cat`, with a graded challenge in six of its ten scrims. Section two covers `find`, `grep`, `sed`, `mv`, `cp`, `wc`, `sort`, `uniq` and the pipe, but Scrimba's browser terminal cannot run most of those, so it is slides plus a walkthrough for setting up a terminal on your own computer. The framing is a running joke: you are the unpaid intern at Ajo's geography quiz game startup, tidying up the file system after a chaotic night of brainstorming.

If you have never typed a command, take it. It costs nothing, the first hour is well drilled, and it removes the fear that makes Git and Node courses feel harder than they are. If you already move around a terminal comfortably, you will learn nothing new here.
