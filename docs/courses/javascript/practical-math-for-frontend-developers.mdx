...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="App Layout and CSS calc() Introduction at 0:59. This one broken layout is why the lesson reaches for calc() at all."
/>
...
<Screenshot
  ...
  width={1046}
  height={588}
  caption="At 2:23 of Your roll() Function, this is the only slide-driven algebra in the course, and the general min-max version follows these same three steps two minutes later."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="Calculate Total / Apply Tax Rate at 1:29. The accumulator is a string here: 0 + '9.08' + '7.30' comes out as 09.087.30 in the console, fixed with parseFloat a few seconds later."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="At 7:43 of Generate, Display, and Track Expenses, every number on screen, the income, the rent, the expense count, and each amount, came from roll()."
/>
...
"That's all Folks" is Ryan's one-minute recap and a pointer to his Discord and social links in the scrim's HTML. "Congratulations" is a Scrimba house scrim in which Leanne pitches the [Frontend Developer Path](/docs/paths/frontend-developer-path/); the figures she quotes (around seventy hours, a Pac-Man game, a Netflix clone) describe the path as it was in 2020, not the current one. "How to Utilize Your Certificate" is Per Borgen's standard 53-second reminder to add the certificate to LinkedIn.
...
Pro also covers the career paths, none of which include this course, and the Pro-only channels on Scrimba's Discord (the pricing page lists basic Discord access as free, so the server itself is not gated). See <AffiliateLink href="https://scrimba.com/our-pricing">current plans</AffiliateLink> for what Pro costs in your region. If you already have Pro, the course is included; I would not subscribe for this course alone.
...
Scrimba does not list it under any career path; it is a standalone course. It is a one-evening top-up between [Learn JavaScript](/docs/courses/javascript/learn-javascript/) and the React track: the `reduce` and `map` patterns reappear constantly in [Learn React](/docs/courses/react/learn-react/), and the CSS variables and `calc()` work pairs with [Learn CSS Variables](/docs/courses/css/css-variables/). If the course leaves you wanting real algorithmic depth, [Data Structures and Algorithms](/docs/courses/javascript/data-structures-and-algorithms/) is the next step, not this one repeated.
