...
Gary Simon's design-to-code course on Scrimba: five Figma mockups, each one built as a working website in about 3.6 hours of video. I expanded every module and read the transcripts of 34 of its 46 scrims (at least a third of every module, every challenge brief, and the closing lesson) with a Pro account in September 2026. This is what is inside, including the JavaScript the title leaves out.
...
Build Websites with Figma, HTML, and CSS is a Pro course, about 3.6 hours long, taught by Gary Simon of the Design Course YouTube channel. Each of its five modules is one Figma design: a card, a landing page, an analytics dashboard, a car listings page, and a drone racing event site. You open the mockup in Figma, try to build it yourself, then watch Gary build it mobile first with media queries. Three of the five projects also use a few lines of vanilla JavaScript for a slide-out menu, a pop-up filter panel, and a like button, so "HTML and CSS" undersells it slightly. If you can already write basic HTML and CSS and want to get good at turning a designer's file into a responsive page, this is the most direct practice in the catalog. The first eight scrims are free to preview.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    alt="Title card of the introduction to Scrimba's Build Websites with Figma, HTML, and CSS course, reading From Figma to Code, Making High Quality Mockups a Reality in the browser, course teacher Gary Simon"
    width={1279}
    height={720}
    caption="The course intro: Gary's own name for it is From Figma to Code."
    source=""
  />
  <Screenshot
    src="/img/scrimba/build-websites-with-figma-html-and-css/chapter-1-simple-card-title-card.webp"
    alt="Title card of the first challenge in Scrimba's Build Websites with Figma, HTML, and CSS course, reading A Simple Card, Make this Card a Reality in the Browser"
    width={1279}
    height={720}
    caption="Module 1: the simple card, your first build."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 3: the dashboard. Every challenge intro slide links to its Figma prototype."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 5: the drone event site, the final and longest project."
...
Two caveats. First, the course dates from around 2021. Gary calls `clamp()` and `aspect-ratio` "relatively new" and notes that `grid-gap` has just been renamed `gap`. A few of his cautions about browser support are out of date, and nothing here touches container queries or newer layout features. Second, it is a Pro course, and the Figma files need a free Figma account to inspect. Neither is a reason to skip it, but you should know before you start.
...
<Screenshot
  ...
  alt="Scrimba Build Websites with Figma, HTML, and CSS lesson Simple Card Finishing Up: the editor shows index.css with p.title, p.description and a media query for min-width 550px setting a 600px max-width and a 9rem photo, while the preview on the right shows the finished mobile card with a mountain photo, the heading An Above Average Experience and a description"
  width={1400}
  height={660}
  caption="Simple Card, Finishing Up, paused at 4:49. The mobile card on the right already matches the Figma file; the 550px media query being written on the left is what turns it into two columns on wider screens."
/>
...
<Screenshot
  ...
  alt="Scrimba Build Websites with Figma, HTML, and CSS lesson JS: Adding functionality to the nav bar: the editor shows thirteen lines of index.js with menuBtn, backBtn and menu constants and two click listeners that set menu.style.transform to translateX(0) and translateX(-100%), while the preview shows the dark dashboard with the sidebar navigation slid open over it, Home highlighted in blue above My Stats, Members, Settings and Help"
  width={1400}
  height={660}
  caption="The only JavaScript in module 3, paused at 3:13. Thirteen lines: grab three elements, set a transform on click. The sidebar on the right has just slid in over the dashboard."
/>
...
<Screenshot
  ...
  alt="Scrimba Build Websites with Figma, HTML, and CSS lesson Styling the Photo Gallery: the editor shows .gallery-container with display grid and grid-template-areas of dot a b, c d dot, dot e dot, plus object-fit cover on the images and nth-of-type rules assigning grid areas, while the preview on the right shows the Last Year's Footage section with five drone photos arranged in a staggered grid above the registration form"
  width={1400}
  height={660}
  caption="Styling the Photo Gallery, paused at 5:26. The three quoted strings on the left are the grid, dots marking empty cells; the five photos on the right have just snapped into that shape after each image got its grid-area."
/>
...
<Screenshot
  ...
  alt="Scrimba Build Websites with Figma, HTML, and CSS lesson Finishing the Project, with the preview pane enlarged to desktop width: the finished drone site's quote section reads The stadium is simply unreal, Scrimba Hall will make this the best drone racing event ever, period, attributed to Anonymous Person, over a faded stadium photo, with the photo gallery overlapping its bottom right corner and the heading Last Year's below"
  width={900}
  height={624}
...
Scrimba's course page lists it under the [Frontend Developer Path](/docs/paths/frontend-developer-path/) and the [Fullstack Developer Path](/docs/paths/fullstack-developer-path/). It is the natural project course after [Learn HTML and CSS](/docs/courses/css/html-and-css/), and pairs well with [Learn Responsive Web Design](/docs/courses/css/responsive-web-design/) and [Learn CSS Grid](/docs/courses/css/css-grid/), which teach the techniques this course applies. Gary's own follow-up, plugged in the last scrim, is his [UI Design](/docs/courses/css/ui-design/) course, which teaches the Figma side of the same workflow.
...
<AffiliateLink href="https://scrimba.com/build-websites-with-figma-html-and-css-c02f" variant="button">
  Preview Build Websites with Figma, HTML, and CSS on Scrimba
</AffiliateLink>
