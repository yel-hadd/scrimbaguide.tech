...
Jonathan Lee Martin's one hour Pro course on a single sorting algorithm, rewritten six ways in JavaScript. I read the transcript of every scrim and opened the code in each implementation lesson with a Pro account in September 2026; this page is what the course actually contains, not the listing rewritten.
...
Data Structures and Algorithms: Merge Sort is a 64 minute, Intermediate, Pro course taught by Jonathan Lee Martin. It is a flat list of eleven scrims. One is the intro and one is a ten minute explanation of how merge sort works and why it beats selection sort. Seven are implementation lessons where you rewrite the same algorithm against a small test file, and two are closing clips. Five of the seven implementation lessons are challenges: the recording stops, you write the code, then he shows his version. It is the direct sequel to his [Binary Search](/docs/courses/javascript/data-structures-and-algorithms-binary-search/) course and reuses the ArrayView helper from it. If you already write JavaScript comfortably and want to practise writing one algorithm well, it is a good hour. If you want a survey of data structures, this is the wrong course.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="The course's only title card, from the intro scrim. The course has no chapters, so there is one."
...
<Screenshot
  ...
  alt="Slide from the Merge Sort lesson in Scrimba's Data Structures and Algorithms: Merge Sort course showing a recursion tree: the list 2,8,5,3,4,8,6 split into 2,8,5 and 3,4,8,6, then into 2 and 8,5 and 3,4 and 8,6, with the single element list 2 highlighted in yellow"
  width={1279}
  height={720}
...
<Screenshot
  ...
  alt="Scrimba Data Structures and Algorithms: Merge Sort, Imperative lesson paused at the challenge brief: the test runner popup shows passes 0, failures 1, the test sorts an array marked with a red cross, and an AssertionError expecting 1,2 but receiving 2,1, with the blurred editor behind it"
  width={1400}
  height={660}
...
The target is the merge loop. Treat `left` and `right` as queues: replace the for loop with `array.map`, ignore the mapped element, and use a ternary that calls `left.shift()` or `right.shift()`. The `i`, `j` and `k` counters disappear. He is honest that `shift` moves every remaining element one slot left each call, so "this solution is not as efficient as the previous one. But the conceptual model is great."

<Screenshot
  src="/img/scrimba/data-structures-and-algorithms-merge-sort/queue-map-ternary-shift-lesson.webp"
  alt="Scrimba Data Structures and Algorithms: Merge Sort, Queue lesson at 6:22: the editor shows mergesortWithQueue with a Divide, Conquer and Combine comment, and the combine step is return array.map with a ternary calling left.shift() or right.shift()"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Data Structures and Algorithms: Merge Sort, Simple lesson at 6:21: the editor shows the merge function wrapping left and right in Cursor and looping with times, the five line mergesortSimple that calls split and merge, and a mergesortSimpleWithArrayView wrapper as the default export, with lodash.times 4.3.2 in the dependencies panel"
  width={1400}
  height={660}
...
The 64 minutes is video runtime, and about 44 of those are the implementation lessons. Five of them stop and wait for you. If you actually write each version before watching the solution, which is the only way this course does anything, plan two to three hours. The imperative version alone can take twenty minutes of edge case debugging if you have not written a merge before, and the cursor lesson takes longer to understand than to type. One sitting of two hours, or two of one hour, is realistic. If you only watch, it is an hour, and you will have learned very little.
...
<AffiliateLink href="https://scrimba.com/data-structures-and-algorithms-merge-sort-c01b" variant="button">
  Open Data Structures and Algorithms: Merge Sort on Scrimba
</AffiliateLink>
