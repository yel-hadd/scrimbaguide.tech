...
Scrimba's free course on Imba, the language Scrimba itself is written in, taught by Nathan Manousos across 2.9 hours and 43 short scrims. I read the transcript of every scrim and the code in each one with a Pro account in September 2026; this page is what the course actually contains, including the parts the listing does not mention.
...
Learn Imba is a free, intermediate, 2.9 hour course on Imba, a language that compiles to JavaScript and has HTML tags and CSS built into its syntax. Nathan Manousos, a designer and developer who has shipped a commercial product in Imba, teaches it as a flat list of 43 scrims. About 40 minutes cover syntax, an hour covers tags, styling and events, and the last hour builds the Dopamine Box, a habit tracker with an icon picker, localStorage persistence and a celebration sound. Twenty-six of the scrims end in a "give that a try" challenge. Nothing in the course is Pro-gated except the certificate.

It is worth two or three evenings if you already write JavaScript and are curious how a language can fold components and styling into itself. It is not a career skill, and the course never claims to be one.
...
Scrimba presents this course as one flat list of 43 scrims with no modules, so the six parts above are my grouping, not Scrimba's; the 43 scrims and 2.9 hours match the course listing and its structured data exactly, and the part durations are the sum of the scrim lengths shown in each lesson's header.
...
<Screenshot
  ...
  alt="Scrimba's Conditionals scrim in Learn Imba paused at 5:50: index.imba declares num, done and isWeekend? variables and a challenge comment describing what to log on weekends and weekdays, with an empty preview pane"
  width={1400}
  height={684}
...
<Screenshot
  ...
  alt="Scrimba's Reset habits and use imba.commit() scrim paused at 2:02: index.imba shows toggleItem counting remaining habits, playing celebrateSound and calling resetAll inside a setTimeout, with the finished habit tracker in the preview showing Toggle, Reset All and Clear Data icons"
  width={1400}
  height={684}
