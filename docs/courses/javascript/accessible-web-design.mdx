...
Scrimba's short accessibility course, taught by Fredrik Ridderfalk in 96 minutes of scrims, most of them challenges on a fictional drone grocery site called Skynet Eats. It has 27 scrims and no modules, so I opened every one and read the transcript of each with a Pro account in September 2026. This page is what is in there, including the parts that are thinner than the listing suggests.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course data does not list it inside a path, though Fredrik's teacher bio says the course "launched through Scrimba's Frontend Developer Path"; I did not check the path page to confirm where it sits today.*

## Quick answer

Learn Accessible Web Design is a Pro course of 27 short scrims, 96 minutes in total, rated Intermediate. You take a site with contrast, alt text, link, form, heading and structure problems and fix it one topic at a time, then find and fix every issue on a fresh contact page as a final exam. It covers contrast, alt text, links, labels, landmarks, headings, rem units, ARIA roles and live regions, and skip links. It does not cover screen reader testing or audit tooling. If you write HTML and CSS but have never checked your work for accessibility, it is a fast fix for that gap. If you already know why placeholders are not labels, you will finish it in an hour and learn little.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={665}
    caption="The opening slide. The course has no chapters, so this is the only title card."
    source=""
  />
  <Screenshot
    src="/img/scrimba/learn-accessible-web-design/wcag-levels-title-card.webp"
    alt="Slide from the Aside: Text contrast scrim in Scrimba's Learn Accessible Web Design course showing three blobs labelled A, AA and AAA, with sparkles around AAA"
    width={1279}
    height={720}
    caption="The WCAG levels slide from scrim 3. The course pushes for AAA wherever possible."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="The intro's closing slide. The superhero framing runs through the whole course."
...
<Screenshot
  ...
  alt="Scrimba Learn Accessible Web Design lesson Text contrast paused at 0:17: the preview shows the Skynet Eats homepage with the green word Eats sitting on top of a green landscape illustration and dark green footer links on a dark grey footer, with a small AA and AAA requirements slide in the corner"
  width={1400}
  height={660}
  caption="Scrim 5 at 0:17, the state you are handed. The green Eats blends into the green illustration behind it, and the footer links are dark green on dark grey. Your job is to bring both up to AAA before Fredrik shows his fix."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Accessible Web Design lesson ARIA at 3:59: the editor shows index.html for the Code Poetry site with a nav list and a div with id toggleTheme carrying role switch, aria-checked false and tabindex 0, and a small preview of the page in the corner"
  width={1400}
  height={679}
  caption="Scrim 16 at 3:59, the solution: role switch so assistive tech knows what it is, aria-checked so it can announce the state, and tabindex 0 so the keyboard can reach it. Note the skip link on line 10, which you build later."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Accessible Web Design lesson Final challenge part 1 at 0:27: the editor shows a CSS comment reading Challenge: Identify all accessibility issues using all your newly acquired knowledge, with six empty dashes to fill in, and the preview shows a Reach Out contact form with faint grey placeholder text, no labels and a button that says Done"
  width={1400}
  height={660}
  caption="Scrim 23 at 0:27. The blank dashes are yours to fill. Everything wrong with this page is something you fixed earlier: faint placeholders, an image with no alt, inputs with no labels, a vague button, a div where main should be, a page that starts at h2, and px font sizes."
/>
