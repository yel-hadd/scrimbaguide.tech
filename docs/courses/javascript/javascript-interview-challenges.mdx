...
Treasure Porth's Pro-only drill pack: 2.3 hours of short coding problems of the kind frontend interviewers hand you on a whiteboard or in a shared editor. I read the transcript of every one of the 62 scrims with a Pro account in September 2026, opened the code in each challenge, and paused a handful of solutions to see what is on screen. This is what is actually in the course, not the listing rewritten.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's own course page says it is part of the Front-End Developer Career Path, and the catalog data also lists it under the Fullstack Developer Path; see our [Frontend Developer Path](/docs/paths/frontend-developer-path/) and [Fullstack Developer Path](/docs/paths/fullstack-developer-path/) pages.*

## Quick answer

JavaScript Interview Challenges is a 2.3-hour, intermediate, Pro-only course by Treasure Porth. It is not a lecture course. It is 28 challenge scrims, each followed by one or more solution scrims, covering string methods, array methods, objects, `map`, `filter`, `reduce`, `sort`, and two small `fetch` exercises against Scrimba's emoji API. The classics are all here: FizzBuzz (renamed), reverse a string, anagrams, palindromes, remove duplicates, flatten an array, count character frequency.

Treasure writes pseudo code before nearly every solution and says out loud why she picks one approach over another. If you can already write JavaScript and have interviews coming, it is a good two-week warm-up. If you are still learning the language, it will frustrate you before it helps.
...
Skip it if you are still learning the language: the course never explains what a `for` loop or an array method is, it only reminds you. Do [Learn JavaScript](/docs/courses/javascript/learn-javascript/) first and come back. Skip it too if your interviews are LeetCode-style: there is one honest conversation about nested loops and one about brute force versus optimal, and no Big O, recursion, or data structures beyond arrays and objects. And if your interviews are React-specific, [React Interview Questions](/docs/courses/react/react-interview-questions/) is the better target.
