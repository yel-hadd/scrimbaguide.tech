...
Scrimba's Pro course on the security work that lands on a working developer's desk: threat modelling, login systems, untrusted input, and traffic limits. I expanded every module and read the transcripts of 36 of its 81 scrims, at least a third of every module, with a Pro account in September 2026. This page is what is actually in there, not the listing rewritten.

*This page is part of our [JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course page does not list it under a path, but Jonathan Hill's teacher card says he created the cybersecurity material in the [Backend Developer Path](/docs/paths/backend-developer-path/), and every code example runs on Node.*

## Quick answer

Learn Cybersecurity is a 5 hour, Intermediate, Pro course split between two teachers. Rachel Johnson teaches the first two modules with slides and short reasoning exercises: the STRIDE and OWASP threat models, then the three ways an app can know who you are (sessions, tokens, and "sign in with Google"). Jonathan Hill teaches the last two with real Express code: you run an XSS attack and a SQL injection against a deliberately broken registration form, fix it with Zod, then build rate limiters and a throttle with `express-rate-limit` and `express-slow-down`.

It is application security for people who already build backends, not a cybersecurity career course. There is no network security, no penetration testing tooling, and no certification prep. If you write Node APIs and have never been taught why a cookie should only hold a session ID, or what a 429 status is for, it is the most practical five hours on the subject in the catalog.
...
The honest caveat is the name. This is developer-facing app security. It will not make you a security engineer, and it only covers a slice of the OWASP top ten in code (XSS, injection, and denial of service). Rate limiting gets almost as much time as authentication, which tells you what the course cares about most: protecting an API you have already built.
...
<Screenshot
  ...
  alt="Scrimba's SQL Injection part 2 scrim paused at 3:53: server.ts shows the challenge brief to craft an email input that drops the users table, and the Create Your Account form in the preview holds the payload '); DROP TABLE users; -- with the banner reading We currently have 0 signed up users"
  width={1400}
  height={684}
...
<Screenshot
  ...
  alt="Scrimba's Fixed Window Counter, Create the limiter scrim paused at 5:08: server.js configures rateLimit with limit 5 and windowMs 60000 and applies it to the /api/data route, while the terminal below shows requests 14 and 15 answered with 429 Too many requests, please try again later"
  width={1400}
  height={684}
