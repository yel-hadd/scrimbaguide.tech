...
Scrimba's 37 minute Markdown course, taught by Dylan C. Israel and built around one small project: a technical blog post you write on dev.to. I opened every one of its 26 scrims, read the full transcripts and the challenge briefs with a Pro account in September 2026, so this page describes what is in the course, not what the listing says about it.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course page shows no path badge, and the course does not appear in any of the four career paths.*

## Quick answer

Learn Markdown is a Pro course of 26 short scrims, none longer than three minutes, that teaches the text formatting syntax used in README files, GitHub comments, documentation and most blogging tools. Dylan covers headings, paragraphs, bold and italic, blockquotes, lists, code blocks, images and links, then four pieces of "extended" syntax (tables, task lists, automatic URLs). Seven of the scrims are challenges, and every one of them adds a section to a real article you publish on dev.to. The final challenge has you open a pull request that adds your article to a shared GitHub repo.

Scrimba labels it Intermediate. It is not. Nothing here assumes you can code, and the hardest idea in the course is that you can nest an image inside a link. If you have never written Markdown and you want a reason to write your first blog post, it is a pleasant hour. If you already write Markdown, or you are not a Pro subscriber, a free cheat sheet covers the same ground.
