...
Learn Express.js is Scrimba's free, intermediate backend course: about 4 hours across three modules, taught by Tom Chant apart from the solo project brief. You need comfortable JavaScript (array methods, fetch, async/await) but, in Tom's own words, no prior Node or Express, and the one catch is that scrimba.com's own listing promises a few features the lessons don't teach. If you want Node fundamentals first, [Learn Node.js](/docs/courses/javascript/nodejs/) is the natural prerequisite.
