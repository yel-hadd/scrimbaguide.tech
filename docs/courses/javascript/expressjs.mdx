...
Scrimba's free Express course, taught by Tom Chant across about four hours and two projects: a startup-data REST API and a vinyl store with a SQLite database, sessions, and hashed passwords. I opened every module and worked through a sample of lessons from each one with a Pro account in September 2026. This is what is in there, including the parts that the course listing gets wrong.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. It is sequenced into the [Frontend Developer Path](/docs/paths/frontend-developer-path/), the [Fullstack Developer Path](/docs/paths/fullstack-developer-path/), and the [Backend Developer Path](/docs/paths/backend-developer-path/).*

## Quick answer

Learn Express.js is a free, intermediate-level backend course: about 4 hours of video across three modules, 58 scrims by my count, 32 of them marked as challenges. You build Startup Planet, a JSON API with query and path parameter filtering, and then Spiral Sounds, a record shop with an Express server, a SQLite database, a genre dropdown and search, and full sign up, log in, log out, cart, and protected routes using express-session and bcryptjs. Tom Chant, who also teaches Learn Node.js and Advanced JavaScript, teaches all of it apart from the solo project brief. You need comfortable JavaScript (array methods, fetch, async/await) but, in Tom's own words, no Node or Express at all. If you want to understand what a backend framework actually does, rather than copy a boilerplate, this is a good four hours.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 1: the Startup Planet API."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 2: the Spiral Sounds store."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 3: authentication."
...
The caveat: the course page on scrimba.com promises things the course does not contain. The "What you'll build" text lists task queues, real-time notifications with Server-Sent Events, file uploads, and Event Emitters. None of those appears in any of the 58 scrims. What you actually get is routing, middleware, static files, SQLite, sessions, hashing, validation, and protected routes. That is a solid syllabus, but if you came for SSE or uploads, this is not where they are. The Authentication module has an "Outro (new)" scrim and the course was modified in May 2026, so the listing may describe a longer version that was planned or trimmed. I can only review what is there.
...
Lesson counts are the scrims I counted in each expanded module in September 2026: 58, plus two certificate items at the end, which is the 60 the course listing and its structured data report. Scrimba's own module headers say 10, 8, and 15; those match the number of challenge icons per module, not the number of scrims.
...
<Screenshot
  ...
  alt="Scrimba Learn Express.js lesson Filtering by Query Params paused at 6:44: the editor shows the challenge comment with three test-case URLs, the first one highlighted, and Scrimba's Network tool on the right shows a GET request to /api with industry, country and has_mvp query params returning the GreenGrid Energy JSON object"
  width={1400}
  height={629}
  caption="Filtering by Query Params at 6:44. The Network tool on the right is Scrimba's own request client, so you test the API without leaving the scrim: the first test case (renewable energy, Germany, has_mvp=true) returns exactly one object, GreenGrid Energy, as the brief said it should."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Express.js lesson Add Search Functionality paused at 3:21: the editor shows the getProducts controller with a WHERE title LIKE ? OR artist LIKE ? clause and a searchPattern built from percent signs, and the mini browser on the right shows the Spiral Sounds store with the letters ec typed into the search box and three matching records"
  width={1400}
  height={629}
  caption="Add Search Functionality at 3:21, the last challenge of module two. Typing 'ec' into the search box narrows the grid live; the controller on the left builds a '%ec%' pattern and pushes it three times for the title, artist and genre placeholders, which Tom admits 'looks super clunky but it is the way to do it.'"
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Express.js lesson Login paused at 7:18: the editor shows the loginUser function with a SELECT * FROM users WHERE username = ? query, a 401 Invalid credentials response and const isValid = await bcrypt.compare(password, user.password), while the mini browser on the right shows the Spiral Sounds store header reading Welcome, Test Account!"
  width={1400}
  height={629}
  caption="Login at 7:18, right after Tom signs in with the test account and the page redirects. The header now reads Welcome, Test Account! because the frontend asked /api/auth/me and the session cookie answered. The bcrypt.compare line on the left is the whole password check."
/>
...
Every scrim has captions, a timestamped transcript panel (settings menu), subtitles in ten languages, and speed controls. Tom's tone is dry and a little self-deprecating ("okay, I am exaggerating with this GIF"), and he is honest about difficulty: the harder challenges get a "congratulations if you did" afterwards rather than a pretence that everyone sailed through. He also solves challenges in front of you with his own reasoning, not just the answer, which is where the "favor simplicity" advice and the "don't give clues away" security point come from.
...
What Pro does gate is the certificate of completion at the end of the course, the three career paths this course sits in, and the Pro-only channels on Scrimba's Discord (the pricing page lists basic Discord access as a free feature). If you want the certificate on a CV, or you plan to work through the Backend or Fullstack path in order, see <AffiliateLink href="https://scrimba.com/our-pricing">current plans</AffiliateLink> for what that costs where you live. If you just want to learn Express, you do not need to pay.
