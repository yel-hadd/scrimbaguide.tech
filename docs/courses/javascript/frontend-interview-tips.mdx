...
Dylan C. Israel's short Pro course on getting through a frontend interview, from the technical screen to the resume that gets you the call. It is 24 scrims and about 104 minutes of video. I read the transcript of every scrim with a Pro account in September 2026; what follows is what is in the course, not the listing page rewritten.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course page says it is also part of the Frontend Developer Career Path, and it pairs with the coding-focused [JavaScript Interview Challenges](/docs/courses/javascript/javascript-interview-challenges/).*

## Quick answer

Frontend Interview Tips is a 104 minute, Pro-only, intermediate course by Dylan C. Israel, an engineering manager who has run a lot of interviews. Despite the name, more than half of it is technical. Fourteen short scrims each pose one screener question (falsy values, const vs let vs var, promises, CSS selectors, Git, Agile), give you five to ten minutes to research it, then show Dylan's answer in the editor. The rest is the conversation itself (five questions to prepare, five preparation tips, what to ask at the end), three long teardowns of a GitHub profile, a LinkedIn profile and a resume, and a job search plan. If you are about to interview for a first or second frontend job and have never sat on the hiring side, it is worth an afternoon.
...
<Screenshot
  ...
  alt="Scrimba's const vs let vs var scrim in Frontend Interview Tips paused at 3:37: index.js declares const PI = 3.14 and then PI = 3.145, and the console shows TypeError: Assignment to constant variable"
  width={1400}
  height={684}
...
<Screenshot
  ...
  alt="Scrimba's Interview Question: Promises scrim paused at 3:18: getPost logs 1, calls fetch on jsonplaceholder with two chained .then callbacks that log 2, then logs 3, and the console shows 1, 3, 2"
  width={1400}
  height={684}
...
Pro also unlocks the Frontend Developer Career Path this course sits in, and the Pro-only channels on Scrimba's Discord (the pricing page lists basic Discord access as free, so the server itself is not gated). See <AffiliateLink href="https://scrimba.com/our-pricing">current plans</AffiliateLink> for what Pro costs in your region.
...
Skip it if you cannot yet answer "what is a promise" with an example; do [Learn JavaScript](/docs/courses/javascript/learn-javascript/) first. Skip it if your problem is algorithm rounds; [JavaScript Interview Challenges](/docs/courses/javascript/javascript-interview-challenges/) and [Data Structures and Algorithms](/docs/courses/javascript/data-structures-and-algorithms/) cover that. And skip the profile lessons if you have several years of experience; the advice is aimed a rung below you.
