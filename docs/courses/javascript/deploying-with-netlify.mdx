...
Scrimba's free, 23-minute walkthrough of putting a project on the web with Netlify, taught by Treasure Porth. It is ten scrims long, so I opened every one and read the transcript of each with a Pro account in September 2026. This page covers what each scrim shows, what you need installed before it is useful, and where it stops.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course page does not list it under any career path; it is a standalone course.*

## Quick answer

Deploying with Netlify is a free, 23-minute Scrimba course that shows two ways to get a site online: drag a folder onto Netlify's drop page, or connect a GitHub repository so every push to `main` redeploys the site. Treasure Porth covers both for plain HTML/CSS/JS projects and for projects with a build step (Vite, Create React App, Next.js), plus a short look at Netlify's deploy previews on pull requests. There are no coding challenges and nothing to build. If you have a finished project and have never deployed anything, it will get you a live URL in an afternoon. If you already push to GitHub and know what `npm run build` does, you can skip it.
...
<div className="screenshot-grid">
  ...
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Section 1: drag-and-drop deploys with Netlify Drop."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Section 2: continuous deployment from GitHub."
...
<Screenshot
  ...
  alt="Scrimba Deploying with Netlify lesson Drop Deploy a Plain JavaScript Project, paused on a slide of Netlify Drop: a dashed circle labelled Drag and drop your site output folder here above the headline Drag and drop, it's online"
  width={1279}
  height={720}
...
<Screenshot
  ...
  alt="Scrimba Deploying with Netlify lesson Continuous Deployment for a Plain JavaScript Project, paused on GitHub Desktop showing a diff of index.html where the h1 changes from Row Counter to Amazing Row Counter, with a Commit to main button below"
  width={1279}
  height={720}
...
<Screenshot
  ...
  alt="Scrimba Deploying with Netlify lesson Continuous Deployment for a React/Vue/Next.js Project, paused on Netlify's import settings page reading This is a Next.js site, with the build command yarn build and publish directory .next pre-filled above a Deploy site button"
  width={1279}
  height={720}
...
<Screenshot
  ...
  alt="Scrimba Deploying with Netlify lesson Netlify's Deploy Preview, paused on a GitHub pull request where the Netlify bot has posted a table with the latest commit, a deploy log link, a Deploy Preview URL, and a Toggle QR Code option for previewing on mobile"
  width={1279}
  height={720}
...
Skip it if you already deploy from GitHub with any host. There is nothing here you do not know. Skip it too if you need a backend: the course is about static sites and frontend frameworks, and it does not cover serverless functions, environment variables, custom domains, build failures, or Vercel and GitHub Pages as alternatives. Scrimba rates it Intermediate, which I read as a warning that scrims 3, 4, and 7 assume a terminal and Node rather than a comment on difficulty; the drag-and-drop scrims are beginner-level.
