...
description: "Shant Dashjian's free Intro to Vite reviewed from inside: 39 minutes, 14 scrims, React 19 on Vite 6, five challenges, and what Pro actually gates."
...
Intro to Vite is Scrimba's free, 39 minute course on the build tool that most React and Vue projects now start from. Shant Dashjian teaches it as a flat run of 14 scrims. I read the transcript of every scrim with a Pro account in September 2026. This page covers what each scrim teaches, which versions it runs on, and where it stops.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course page does not list it under any career path; it is a standalone elective.*

## Quick answer

Take it if you already use React or Vue and treat `npm run dev`, the `dist/` folder, and `vite.config.js` as magic. By the end you can scaffold a React app from the terminal and explain why a saved edit shows up without a page reload. You can also put a logo in the right folder, read an API key from a `.env` file, build for production, and add a plugin from its docs. Five of the 14 scrims stop for a challenge.

Skip it if you have never written a React component. The course is rated Intermediate because it assumes you can read one and use a terminal, and it never explains either.
...
<Screenshot
  ...
  alt="Scrimba Intro to Vite lesson Create your first React project with Vite, paused at 2:17: the file explorer lists node_modules, public, src, index.html and vite.config.js, the terminal shows VITE v6.2.6 ready in 4204 ms with a Local localhost:5173 URL, and the mini browser on the right shows the default Vite + React page"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Intro to Vite lesson Handle static assets, paused at 1:26: App.jsx imports reactLogo from ./assets/react.svg and viteLogo from /vite.svg, and the terminal shows vite v6.2.6 building for production with 32 modules transformed and a dist/assets/react-CHdo91hT.svg output line"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Intro to Vite lesson Use environment variables, paused at 3:03: App.jsx shows const greeting = import.meta.env.VITE_GREETING, a useState for ipAddress, and a useEffect with a fetchIpAddress function containing the comment Use the environment variable here above const apiUrl equals question mark; tabs for .env and CHALLENGE.md are open and the terminal shows a stopped Vite dev server"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Intro to Vite lesson Extend with plugins, paused at 1:55: package.json is open with the dev script changed to vite --host and react 19.0.0 in dependencies, a vite.config.js tab is open beside it, and the terminal below shows a large QR code printed by vite-plugin-qrcode after npm run dev"
  width={1400}
  height={660}
...
If you follow along on your own machine instead of the sandbox, add time for installing Node and your first `npm create vite@latest`. It is one sitting. Doing it twice, once in the browser and once locally, is a reasonable use of a second hour.
