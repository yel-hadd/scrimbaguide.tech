...
Advanced JavaScript is Scrimba's Pro, intermediate-rated JavaScript course, taught by Tom Chant with Bob Ziroll guesting for the API theory in module four. Tom says in the first minute that "the concepts are broken down into bite sized chunks and we'll be working a lot in the console" rather than one big project. It fills the gap for developers who can already write JavaScript but could not explain the event loop, `this`, prototypes, or why a generator uses `yield`; only the first five scrims are free.
