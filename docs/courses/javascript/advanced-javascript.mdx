...
Scrimba's Pro follow-up to Learn JavaScript, taught by Tom Chant across about 9.8 hours and nine modules that run from the ternary operator to generators. I opened 65 of the 156 scrims with a Pro account in September 2026, at least a third of every module, and read each one's transcript. This page is what is actually in the course, including the counts Scrimba's listing gets wrong.

*This page is part of our [Scrimba JavaScript courses](/docs/courses/javascript/) catalog. Scrimba's course page lists it under the [Frontend Developer Path](/docs/paths/frontend-developer-path/) and the [Fullstack Developer Path](/docs/paths/fullstack-developer-path/).*

## Quick answer

Advanced JavaScript is Scrimba's Pro, intermediate-rated JavaScript course: 9.8 hours of video across nine modules, taught by Tom Chant, with Bob Ziroll guesting for the API theory in module four. I counted 156 scrims in the expanded table of contents. 80 of them carry a challenge icon, and Scrimba's module headers add up to 94 challenges in total.

There is no single project. Tom says so in the first minute: "In this course, we are not building one big project. Instead, the concepts are broken down into bite sized chunks and we'll be working a lot in the console." If you can already write JavaScript but could not explain the event loop, `this`, prototypes, or why a generator uses `yield`, this is the course in the catalog that fills that gap. Only the first five scrims are free.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    alt="Title card of Scrimba's Advanced JavaScript course on an orange and purple gradient, reading Scrimba Pro Course, Advanced JavaScript, Teacher: Tom Chant"
    width={1279}
    height={720}
  ...
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 4: asynchronous JavaScript and APIs, the longest module by scrim count."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 9: closures, recursion, currying, throttling, debouncing, and generators."
...
The second is that the intermediate label is honest for modules one to three and five. Four, seven, and nine are advanced, and Tom warns you in each intro. He says as much at the start of the custom objects module: "some of this stuff is hard. When I first came across it, it made me wanna go and learn Python instead or just become a hermit." Plan for those three modules to take longer than the rest.

The caveat is scope. This is vanilla JavaScript in the console with occasional DOM work: no frameworks, no TypeScript, no build tools, and no project you can put in a portfolio. That is the point of the course, and Tom says so in the intro. If what you want is another app to build, look elsewhere.
...
Lesson counts are the scrims I counted in each expanded module in September 2026 (155, plus a one-minute certificate scrim after module nine). I re-expanded the table of contents for this review and the counts held. Scrimba's numbers measure different things. The "0/14" style figure in each module header is the number of challenges in that module: 94 across the course, and I confirmed it matches the challenge icons. The course listing and its structured data say 207 lessons, which counts the clips inside scrims rather than the scrims you click. When other pages on this site quote 207, that is Scrimba's number.
...
<Screenshot
  ...
  alt="Scrimba Advanced JavaScript lesson Callback Hell paused near the end: the editor shows uploadFile, processFile and notifyUser nested inside each other as arrow-function callbacks, and the console below prints Step 1: Uploading file, Step 2: Processing file, Step 3: Notifying user, All steps completed"
  width={1400}
  height={660}
  caption="Callback Hell at 5:17. Three fake async steps chained the old way, each callback nested inside the previous one, with the console proving the order and the one-second delays. Tom calls the shape 'the pyramid of doom' and rewrites it with promises in the next scrim."
/>
...
<Screenshot
  ...
  alt="Scrimba Advanced JavaScript Super Challenge: Game Characters paused at the brief: the editor shows an empty class Character with a long comment spec asking for a name property, a private health field initialized to 100, a static count, incrementCount and getCount static methods, a health getter and setter, takeDamage, an isAlive getter and getStatus"
  width={1400}
  height={660}
  caption="Game Characters at 1:08. The whole brief lives in comments inside the empty classes: a Character base class with a private #health field, static counters, getters and setters, then Hero and Villain subclasses. Test calls with expected output sit at the bottom of the file."
/>
...
<Screenshot
  ...
  alt="Scrimba Advanced JavaScript Debouncing and Generators Super Challenge paused mid-solution: the editor shows a function* fetchDataGenerator with a while loop that now yields a new Promise wrapping setTimeout, and a handleScroll function that calls generator.next() and appends a section to the page"
  width={1400}
  height={660}
  caption="The Debouncing and Generators super challenge at 4:16, just after the first real bug is fixed: the generator was using return instead of yield. More bugs follow, including a result.done check and an off-by-one in the section counter."
/>
...
Every scrim has captions, a timestamped transcript panel under the settings menu, and subtitles in ten languages. The transcript is what I used to read the course and to pick the moments in the screenshots above. Tom's voice is dry British humor over a fairly fast pace. He speed-types repetitive code ("I'll just speed type the next two functions"), warns you before the hard parts, and is honest about difficulty in a way I found reassuring rather than off-putting. He is also good at saying when not to persist, as in the callback hell scrim.
...
What it does well: it is challenge-first (80 of 156 scrims), every module has a clear concept order, and the super challenges are the closest thing to real problems in the JavaScript catalog. Tom is honest about what is hard, and the transcript panel makes the theory-heavy scrims easy to revisit.
