...
Dylan C. Israel's short Pro course on writing your first JavaScript unit tests with Jasmine, one class and one test file at a time. I opened all 23 scrims and read the transcript of every one with a Pro account in September 2026, so this page describes what is actually in the course rather than its listing.
...
Introduction to Unit Testing is an 86-minute, Pro-only, intermediate course taught by Dylan C. Israel. It uses the Jasmine test framework, loaded as plain script files in the browser. The code under test is a small `User` class: default values, a `fullName` getter, a method that calls `window.alert`, and an async method that calls a user service. Eight of the 21 teaching scrims are challenges. It is a fundamentals course by design. Dylan says in the intro that he does not want you to memorize Jasmine. He wants you to understand grouping, test cases, and arrange-act-assert so you can carry them to Jest or any other framework. Worth it if you write JavaScript and have never written a test. Skip it if you already have.
