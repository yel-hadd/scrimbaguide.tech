...
Learn TypeScript sits on both the [Fullstack Developer Path](/docs/paths/fullstack-developer-path) and the [Backend Developer Path](/docs/paths/backend-developer-path) as the typing layer over JavaScript. It pairs naturally with [Learn React](/docs/courses/react/learn-react), since the React module builds directly on that knowledge, and it complements backend work where typed Express APIs are common.
