...
Scrimba's free TypeScript course: about 4.2 hours in three sections, the first taught by Bob Ziroll and the other two by Rachel Johnson. I opened 41 of its 79 scrims with a Pro account in September 2026, at least a third of every section, and read their transcripts. This is what is inside, who teaches what, and where it stops.
...
Learn TypeScript is a free, intermediate course of about 4.2 hours. Bob Ziroll teaches the 2.1 hour fundamentals section by writing a deliberately buggy pizza restaurant in JavaScript, moving it to TypeScript, and fixing what the compiler flags. Rachel Johnson then spends 52 minutes converting the Assembly: Endgame game from Learn React to TypeScript, and 73 minutes building a typed pet shelter API in Express 5. Around two thirds of the scrims stop for a pause-and-code challenge. If you already write JavaScript and want to know why TypeScript is worth the extra typing, it is the best short answer on Scrimba.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Section 1: TypeScript fundamentals, taught by Bob Ziroll."
    source=""
  />
  <Screenshot
    ...
    width={1377}
    height={775}
    caption="Section 2: TypeScript in React, where Rachel Johnson takes over."
    source=""
  />
  <Screenshot
    ...
    width={1377}
    height={775}
    caption="Section 3: Express and TypeScript, a typed pet shelter API."
...
The first caveat is the name. Scrimba's course title and our catalog data credit Bob Ziroll, and he does teach the fundamentals. The React and Express sections are taught by Rachel Johnson, whose teacher card sits next to Bob's further down the course page. Both are clear, but they have different styles: Bob talks through his reasoning and gets on the odd soapbox ("I'll get off my high horse now"), while Rachel is brisker, with a challenge every second scrim.

The second caveat is depth. Bob says in his conclusion that "what you've watched is everything that we have," and that more topics were planned. Enums get a one-line mention, `unknown` appears only in passing, and in the 41 scrims I opened there were no interfaces, classes, or declaration files; no scrim title mentions them either. Rachel's sections assume you already know React and Express; she says so in the Express intro. If you want those tools taught, take [Learn React](/docs/courses/react/learn-react/) and [Learn Express.js](/docs/courses/javascript/expressjs/) first.
...
<Screenshot
  ...
  alt="Scrimba Learn TypeScript lesson Intro to Pizza app, paused at 9:15: index.js shows placeOrder and completeOrder functions and calls to addNewPizza with a cost property, while the console below prints TypeError: Assignment to constant variable three times"
  width={1400}
  height={630}
...
<Screenshot
  ...
  alt="Scrimba Learn TypeScript lesson Move code to TS, paused at 0:47: index.ts contains the pizza restaurant code pasted from index.js, with red underlines under orderQueue, pizzaObj, pizzaName, cashInRegister, selectedPizza, nextOrderId and orderId, and red error markers in the scrollbar"
  width={1400}
  height={630}
...
<Screenshot
  ...
  alt="Scrimba Learn TypeScript lesson The Final Challenge, paused at 0:30: Keyboard.tsx opens with a comment block reading FINAL CHALLENGE, fully type the Keyboard component, with three numbered steps, and the destructured props alphabet, guessedLetters, currentWord, isGameOver and addGuessedLetter are underlined in red; the file tree shows the components folder and tsconfig files"
  width={1400}
  height={630}
...
Running the project is taught honestly: `node src/index.ts` fails, so you compile with `npx tsc` and run `node dist/index.js`, and a later challenge turns that into an npm `start` script. The package.json I saw shows Express 5.1 and TypeScript 5.9. Requests are sent from a Network panel inside Scrimba's editor, so there is no Postman to install.
...
<Screenshot
  ...
  alt="Scrimba Learn TypeScript lesson String query params, paused at 2:46: the Runner panel shows npm run start failing with error TS2339, Property toLowerCase does not exist on type string | ParsedQs | (string | ParsedQs)[], pointing at pet.species.toLowerCase() === species.toLowerCase() on line 34 of src/index.ts, while the Network panel on the right shows a GET to localhost:8000/?species=cat returning a JSON list of pets"
  width={1400}
  height={630}
