...
This page is part of our [Scrimba React courses](/docs/courses/react/) catalog. It stands alone rather than sitting inside a career path.
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="At 4:19 of useTransition (2), isPending flips true the moment Products is clicked, well before the 1,500-item list is ready."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="The only hint in this useActionState() Part 2 challenge, at 2:10, is the isPending value already sitting on line 7 of the file."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="useActionState() Part 3, about 6:15: typing the word error makes the fake database throw, and the failure now renders in the UI instead of the console."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="The exact failure at 6:00 of use() stops the demo cold: 'async/await is not yet supported in Client Components'."
/>
...
What Pro adds around it: the Certificate of Completion row at the bottom of the table of contents and the Pro-only Discord channels. Basic Discord access is free according to Scrimba's pricing page, so the server itself is not a reason to upgrade. One thing I could not verify: Scrimba caps free accounts at 10 interactive challenges in total, and the three "Challenge with Instant Feedback" scrims here may count toward that cap. See <AffiliateLink href="https://scrimba.com/our-pricing">current plans</AffiliateLink> if the certificate matters to you.
...
Skip it if you have never written React; Bob says as much in the intro, and he assumes you know `useState`, `useEffect`, async functions and what a promise is. If you took the re-recorded [Learn React](/docs/courses/react/learn-react/) you already know the `action` prop; the rest of this course (`useActionState`, `useOptimistic`, `useFormStatus`, `use()`) is still new. And if you are here for the React Compiler or server components, this is not the course: the compiler gets two minutes and server components are not covered.
...
This is a side course rather than a step in a sequence, and it is not part of either career path. The natural moment to take it is right after [Learn React](/docs/courses/react/learn-react/) or [Advanced React](/docs/courses/react/advanced-react/), when the new hooks will make sense in context. If the forms material was the part you liked, [React Router](/docs/courses/react/react-router/) covers the router-side form handling Bob mentions in the Form action scrim.
...
<FAQAccordion items={[
  { q: "Is What's New in React 19 free on Scrimba?", a: "Yes. All 15 scrims, including the three challenges, open on a free account, and there are no Solo Projects to gate. Pro is only needed for the certificate row and the Pro-only Discord channels; basic Discord access is free." },
