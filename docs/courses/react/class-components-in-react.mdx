...
description: "Bob Ziroll's 80 minute Pro course on React class components, reviewed from inside: every scrim read, four challenges, React 17, and who actually needs it."
...
Bob Ziroll's short Pro course on the older, class-based way of writing React: 18 scrims and about 80 minutes of video. I read the transcript of every scrim, opened the code in each one, and worked through the challenges with a Pro account in September 2026. This is what is inside, and who I think should bother.
...
Learn Class Components in React is a Pro course rated Intermediate, taught by Bob Ziroll, with 80 minutes of video across 18 scrims (Scrimba's listing rounds this to 1.3 hours). It teaches the class syntax React used before hooks: `extends React.Component`, `render()`, `this.state` and `this.setState()`, the constructor and `.bind()`, and the three lifecycle methods you meet most often. Every example runs on React 17.0.2. Bob recorded it as an onboarding section for his Advanced React course, and it reads that way: a crash course for people who already know hooks and need to read older code. If you are learning React for the first time, take the free [Learn React](/docs/courses/react/learn-react/) instead and come back only if a codebase forces you to.
...
<Screenshot
  ...
  alt="Scrimba Learn Class Components in React, the convert to a class component with state challenge paused at 0:18: App.js shows a four step challenge comment above a function component using React.useState for a counter, and the preview shows a counter reading 0 with minus and plus buttons"
  width={1400}
  height={660}
...
The section ends with "Updating complex state," a contact card with a favorite star. After a third conversion challenge, Bob shows the one thing classes do better: `this.setState()` merges the object you give it into existing state, so you can drop the spread operator you needed with `useState`. He rates the win honestly: "in reality we only avoided writing one line of code. But that being said, we gotta take the wins when we can get them."
...
<Screenshot
  ...
  alt="Scrimba Learn Class Components in React, the componentDidMount lesson paused at 5:16: App.js shows componentDidMount fetching swapi.dev/api/people/1 and calling this.setState with the character, render returns an h1 with this.state.character.name, the console lists render, componentDidMount, render, and the preview shows Luke Skywalker"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Learn Class Components in React, componentDidUpdate part 2 paused at 2:28: App.js shows componentDidUpdate logging Updated and calling this.getStarWarsCharacter, the console shows the word Updated repeated nine times, and the preview shows a counter at 1 above the name Luke Skywalker"
  width={1400}
  height={660}
...
<AffiliateLink href="https://scrimba.com/learn-class-components-in-react-c0h" variant="button">
  Preview Learn Class Components in React on Scrimba
</AffiliateLink>
