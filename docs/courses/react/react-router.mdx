...
Scrimba's dedicated routing course, taught by Bob Ziroll across about 9.7 hours and 139 scrims, all of it spent building one app called VanLife. I opened every module and read the transcripts of 64 of the 139 scrims, at least a third of each module, with a Pro account in September 2026. This page is what is actually in there.
...
Learn React Router is a Pro, intermediate-level course on React Router 6, including the 6.4 data APIs (loaders, actions, and deferred data). It runs about 9.7 hours across six modules and 139 scrims, and the first five scrims are free previews. You build VanLife, a camper van rental site with a public catalog, a filterable list, a fake login, and a host dashboard that goes three navigation levels deep. Bob's condition for starting is a "pretty solid foundation in React"; if you have finished [Learn React](/docs/courses/react/learn-react/), you have it. If routing is the one gap in your React, this is the most complete treatment of it on Scrimba. It is also more current than the routing lessons in older React courses, because it covers the data router that React Router 7 is built on.
...
The honest caveat is age, and it is a mild one. The scrims pin `react-router-dom` 6.4.3 and 6.4.4 with React 18.2. React Router 7 has since shipped, but its routing model is the 6.4 data router this course teaches, so everything transfers. The one part that has moved is the last module. v7 deprecates `defer()` in favor of returning promises straight from a loader, and the "middleware" Bob says he hopes to record an update about has since landed in v7, first as an unstable API. Neither invalidates a lesson; you will just write slightly shorter code today.

The second caveat is heritage. Bob twice refers to "the full advanced React course," and in the NavLink lesson he says render props are something "you have already learned about" if you are taking it there. This course started life as the routing module of [Advanced React](/docs/courses/react/advanced-react/). If you own that course, you own most of this one. If you want routing on its own, this is the cheaper way in.
...
- One app carried across all 139 scrims, with three levels of nested layouts you actually build.
...
- It duplicates the routing module of Advanced React.
