...
This course started life as the routing module of [Advanced React](/docs/courses/react/advanced-react/). Bob twice refers to "the full advanced React course," and in the NavLink lesson he says render props are something "you have already learned about" if you are taking it there. If you own [Advanced React](/docs/courses/react/advanced-react/), you own most of this one. If you want routing on its own, this is the cheaper way in.
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="A useEffect with an empty dependency array holds this fetch, 7:49 into the second Vans Page challenge, the exact block module 4 later replaces with a loader."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="The temporary workaround at 6:15 of Creating the Host Layout: a child route repeating the /host path so the dashboard renders inside the HostLayout's Outlet."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="A 400 from the server leaves vans undefined, so .map throws: the deliberate crash at 2:08 of Happy Path vs. Sad Path, the one screen every fix through the end of module 4 exists to prevent."
/>
...
<Screenshot
  ...
  width={1400}
  height={660}
  caption="After a successful login, the action redirects you to the page you were trying to reach, here /host/vans/1/pricing: that's redirectTo in VanLife at 4:15, with the loader and action in the same file as the component."
/>
...
Pro also unlocks the certificate at the end and the Pro-only Discord channels. Bob points you to the Scrimba Discord in the outro; Scrimba's pricing page lists basic Discord access under the free plan, so the server itself is not a Pro perk. See <AffiliateLink href="https://scrimba.com/our-pricing">current plans</AffiliateLink> for what Pro costs where you are.
...
Take it straight after [Learn React](/docs/courses/react/learn-react/), whose Chef Claude and Meme Generator projects fetch in `useEffect` the way VanLife does in module 1. This course stands alone; it is not part of either career path. The Netlify and Firebase lessons overlap the standalone [Deploying with Netlify](/docs/courses/javascript/deploying-with-netlify/) and [Learn Firebase](/docs/courses/javascript/firebase/) courses, and the form-actions lessons pair well with [What's New in React 19](/docs/courses/react/whats-new-in-react-19/), where Bob covers React's own form actions.
