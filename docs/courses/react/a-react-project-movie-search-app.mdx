...
A one hour Pro project with James Q Quick: a search box, a call to The Movie Database API, and a list of poster cards, built in React with hooks. I read the transcript of every one of its 13 scrims and the code in each one with a Pro account in September 2026, so this page describes what is in the course, including the parts that have aged.
...
Build a React Project: Movie Search App is a 56 minute, Intermediate, Pro-only build taught by James Q Quick. You write two functional components, `SearchMovies` and `MovieCard`, wire a form to `useState`, fetch results from The Movie Database (TMDB) with `async`/`await`, and map the response to styled cards. Three of the 13 scrims are challenges where the recording stops and you write the component or the state yourself. The first four scrims are free samples.

It is a good ninety minutes if you have just finished a React fundamentals course and want to rehearse the input, fetch, render loop once on your own. It is not a place to learn React from scratch, and it runs on React 16.13 with `ReactDOM.render`, so treat the code as a drill rather than a template for a new project.
