...
Per Borgen's Learn CSS Grid is an hour-long course made of 17 short scrims. A scrim is Scrimba's interactive screencast: a recorded lesson you can pause at any moment to edit the code yourself. The 17 take you from `display: grid` to a 12-column page layout, a responsive image grid, and named grid lines. I opened all 17 scrims with a Pro account in September 2026 and read the transcript of every one that has a transcript panel, along with the code in each. The three slide-only clips at the end have no transcript. This is what is in there, including the parts that have aged.
...
Learn CSS Grid is a Pro, intermediate-level course of about 63 minutes, taught by Per Borgen, Scrimba's co-founder. The table of contents is a flat list, but Per's own transitions split it into four parts.

A five-scrim first section builds a website layout. A three-scrim second section builds a responsive image grid. Five bonus scrims cover named lines, alignment, auto-fit versus auto-fill, and grid with flexbox. A wrap-up holds the Pro solo project, a BBC News clone. The first three scrims are free samples.

Nothing here is a coding challenge in the modern Scrimba sense. The early lessons end with Per telling you to pause and change the values yourself, and the later ones assume you will. If you know basic CSS and want grid to click in an evening, this is the right hour. If you want a full CSS education, it is not that, and does not pretend to be.

By the end you will be able to lay out a page header, sidebar, content area, and footer on a 12-column grid. You will be able to make an image gallery that reflows as the window narrows. And you will be able to explain the difference between `auto-fit` and `auto-fill` to someone who has argued about it.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1277}
    height={719}
    caption="The only title card in the course: the Pro solo project, a BBC News article grid."
...
<Screenshot
  ...
  alt="Scrimba Learn CSS Grid lesson Your first grid paused at 2:27: the editor shows a .container rule with display: grid, grid-template-columns: 100px auto, grid-template-rows: 50px 50px 200px and grid-gap: 3px, and the preview on the right shows six colored numbered cells in two columns and three rows, with the last row much taller"
  width={1400}
  height={635}
...
<Screenshot
  ...
  alt="Scrimba Learn CSS Grid lesson Positioning items paused at 6:01: index.css shows a 12-column grid with repeat(12, 1fr), header and content set to grid-column: 2 / -1, footer to 1 / -1 and menu to grid-row: 1 / 3, while the preview shows a red MENU block spanning the left of the first two rows beside HEADER and CONTENT, with FOOTER along the bottom"
  width={1400}
  height={635}
...
<Screenshot
  ...
  alt="Scrimba Learn CSS Grid lesson Template areas paused at 3:56: the editor shows grid-template-areas with three strings of letters, h for header, m and c for menu and content, f for footer, with a dot at each end of the first and last strings, and the preview shows the header and footer inset from the corners while the menu and content fill the middle row"
  width={1400}
  height={635}
