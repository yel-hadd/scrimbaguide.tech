...
Intro to UI Design Fundamentals is a free, flat course of 20 scrims: seven design principles, each followed by a "Design Challenge" where you fix a broken layout in `index.css`. There's no Figma, no design tool, and no JavaScript, just an HTML page and a CSS file with a comment marking where your fixes go. It's rated Intermediate because it assumes you already write CSS, not because the material is hard, which makes it the fastest fix in the catalog if your pages look slightly wrong and you don't know why.
