...
Working HTML and CSS, including flexbox, and enough Tailwind to read a class like `md:grid-cols-2` without help. Rachel points to Scrimba's [Learn Tailwind CSS](/docs/courses/css/tailwind-css/) course twice in the first ten minutes as the place to get that. There is no JavaScript to write: `config.js` is a single object literal. Nothing to install either, because Tailwind loads from the CDN inside the Scrimba editor.

## Where it fits

Take it straight after [Learn Tailwind CSS](/docs/courses/css/tailwind-css/). This course is not part of any Scrimba career path; it is a short supplementary build on its own. If you liked the transforms and transitions asides, [CSS Animations](/docs/courses/css/css-animations/) goes much further on that topic, and [Learn Responsive Web Design](/docs/courses/css/responsive-web-design/) covers the breakpoint thinking in plain CSS.
