...
A 75 minute Scrimba project with Rachel Johnson: you take a client's two mockups (one desktop, one mobile) and build a product card for a single orange, priced at $1.35, using nothing but Tailwind utility classes. I read the transcript of every scrim, all 23 of them plus the three Scrimba housekeeping clips at the end, with a Pro account in September 2026. This page is what is actually in the course, not the listing rewritten.

*This page is part of our [Scrimba CSS courses](/docs/courses/css/) catalog. Scrimba's course page lists it under the Frontend Developer Path, the Fullstack Developer Path and the AI Engineer Path.*

## Quick answer

This is a short, Pro-only, intermediate project, not a Tailwind course. It assumes you already know what `p-4`, `flex` and `md:` mean. Its 75 minutes go on the things a beginner course skips: editing the `tailwind.config` object, loading custom fonts, gradients, list markers, grid, responsive background images, transforms and transitions, and square-bracket arbitrary values. Every concept is taught twice: once on a throwaway demo page (an "Aside" scrim), then again as a task on the real card. The first four scrims are free previews. If you have finished [Learn Tailwind CSS](/docs/courses/css/tailwind-css/) and want one evening of practice that ends in a finished component, this is a good pick. If you have never used Tailwind, start there instead.
...
The honest caveat is size. Seventy five minutes is not much, the card has maybe a dozen elements, and the "intermediate" label mostly means "not a beginner course," not "hard." Several tasks are one class long. If you want depth, the [Learn Tailwind CSS](/docs/courses/css/tailwind-css/) course is the better buy. This one is a rep.

One more thing worth knowing: the course uses the Tailwind CDN script and a `tailwind.config` object in a `config.js` file. That is the Tailwind 3 way of customizing the framework. Tailwind 4 moved most of that configuration into CSS, so what you learn here about the config object transfers as an idea (extend the theme, name your own colours and fonts) more than as exact syntax. The utility classes themselves are the same.
