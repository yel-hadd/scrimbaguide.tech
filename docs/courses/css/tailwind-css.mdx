...
Scrimba's Tailwind course, taught by Rachel Johnson: about 2.2 hours that walk you from `w-64` and `bg-slate-700` to a responsive blog and five styling challenges. I opened both modules and read the transcript of every scrim with a Pro account in September 2026; what follows is what is in there, not the listing rewritten.

*This page is part of our [Scrimba CSS courses](/docs/courses/css/) catalog. Scrimba's course page lists it under the [Frontend Developer Path](/docs/paths/frontend-developer-path/), the [Fullstack Developer Path](/docs/paths/fullstack-developer-path/), and the [AI Engineer Path](/docs/paths/ai-engineer-path/).*

## Quick answer

Learn Tailwind CSS is a short, Pro-tier course: 2.2 hours across two modules and 33 scrims, taught by Rachel Johnson. The first module teaches Tailwind one utility family at a time (sizing, colors, spacing, typography, flexbox, borders, modifiers, breakpoints) and applies each one to a plant-lovers' blog you style from a mockup. The second module is five challenges, each with a mockup brief and a recorded solution: a Pomodoro layout, a cattery testimonials section, a neon pricing table, a tea subscription preferences form, and a 404 page with header and footer. It assumes you already know CSS, flexbox in particular. If you do, it is a quick, clearly sequenced way to get comfortable reading and writing utility classes.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 1: the course intro slide."
    source=""
  />
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="Module 2: the five challenges."
...
Two caveats. First, it is short and it knows it: the outro says "this course was just an introduction to Tailwind" and points you to the docs for everything it skipped (grid utilities, custom configuration, dark mode, arbitrary values, and any build tooling). Second, the CDN it uses is the Tailwind 3 Play CDN. If you install Tailwind 4 locally afterwards, the classes still work but the setup looks different, and the course does not cover installing anything.
...
<Screenshot
  ...
  alt="Scrimba Learn Tailwind CSS lesson Header and Footer paused at 0:23: the editor shows a YOUR TASKS comment listing header and footer colors, logo, padding and card widths above an unstyled index.html with the cdn.tailwindcss.com script, while the preview on the right shows the raw blog: two nav links, a plant photo and an author avatar with no styling"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Learn Tailwind CSS Flexbox lesson paused at 5:05: index.html shows a section with class flex gap-8 for the latte prices and a section with class flex flex-wrap for the extras, while the preview shows a dark green cafe menu on a crimson page with the extras run together as CreamWaferShortbreadMarshmellow"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Learn Tailwind CSS Desktop layout lesson paused at 3:55: the preview pane is enlarged over the editor and shows the finished plant blog with a dark slate header, a BLOG heading, and three white cards in a row with plant photos, author avatars, titles, three-line excerpts and like counts"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba Learn Tailwind CSS Pricing Table Challenge Possible Solution paused at 7:41: the enlarged preview shows the finished TerminalLux pricing table on black with three cyan-bordered cards, Basic Opal $49, Platinum Debug $149 and Diamond Console $249, the middle Buy Now button filled cyan on hover, with sparkle icons above and below"
  width={1400}
  height={660}
...
<AffiliateLink href="https://scrimba.com/learn-tailwind-css-c010" variant="button">
  Preview Learn Tailwind CSS on Scrimba
</AffiliateLink>
