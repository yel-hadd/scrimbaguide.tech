...
Scrimba's Bootstrap course is a 56 minute tour of Bootstrap 4 by Neil Rowe: ten short screencasts, no modules, and nothing to build. I read the transcript of every scrim (all 13, with a Pro account in September 2026) and typed along with the longer ones. This page is what is inside, including the parts that have aged since it was recorded.

*This page is part of our [Scrimba CSS courses](/docs/courses/css/) catalog. Scrimba's course page does not list it under any career path.*

## Quick answer

Learn Bootstrap is a Pro course, labeled Intermediate, made of 13 scrims that add up to about 56 minutes. A scrim is Scrimba's unit of a lesson: a recorded screencast whose code you can pause and edit. Ten of the 13 are Neil Rowe's lessons, an intro plus nine components. The other three are Scrimba house scrims tacked onto the end. Four scrims carry a SAMPLE badge and play without a subscription.

Every lesson loads Bootstrap 4.0.0 from a CDN, a content delivery network that serves the framework's files from a link tag rather than a download. That is the January 2018 release, not Bootstrap 5. What you get is a fast, clear walkthrough of the classes you will meet in a Bootstrap codebase, and that is all you get.

If you need Bootstrap for a job, an hour here is well spent. If you are choosing a CSS framework for new work, look at [Tailwind](/docs/courses/css/tailwind-css/) first.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1279}
    height={720}
    caption="The only title card in the course, from the introduction scrim. There are no chapters, so there are no chapter cards."
...
<Screenshot
  ...
  alt="Scrimba Learn Bootstrap lesson Responsive Grid Systems paused at 6:45: the editor shows a row of four columns, the first already changed to class col-sm-6 col-md-3 and the other three still col-6, while the preview window on the right shows the four boxes laid out as a two by two grid"
  width={1400}
  height={660}
  caption="6:45 of the grid lesson, mid-edit. Line 20 has the col-sm-6 col-md-3 combination, lines 21 to 23 are still col-6, and the preview is still two by two because it has not been refreshed yet. This is the moment to pause and copy the class yourself before Neil does it."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Bootstrap lesson Responsive Navbars paused at 5:33: the editor is dimmed behind a centered preview window showing a light navbar with the brand My Website on the left and a burger menu icon on the right"
  width={1400}
  height={660}
  caption="5:33 of the navbar lesson, just after the navbar-toggler-icon class goes in. Scrimba dims the editor when the recording focuses the preview; at this narrow width the four links have collapsed behind the burger."
/>
...
<Screenshot
  ...
  alt="Scrimba Learn Bootstrap lesson Cards at 0:00: the editor shows a card with card-img-top pointing at placeimg.com, a card-body with title and text, a list-group list-group-flush with three items and two card-link anchors, while the preview shows the rendered card with the broken image alt text Card image cap where the photo should be"
  width={1400}
  height={660}
  caption="The Cards lesson at 0:00, before Neil clears the file. The img src on line 16 points at placeimg.com, a placeholder service that shut down, so the preview shows the alt text where the header photo used to load. Swap in any other image URL and the rest of the card works."
/>
