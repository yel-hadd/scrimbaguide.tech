...
Scrimba's big UI design course, taught by Gary Simon of the DesignCourse YouTube channel: about 8.6 hours of fixing ugly layouts and then building good ones, all in HTML and CSS. I opened every module and read the transcripts of 45 of its 101 scrims (at least a third of every module, every chapter intro, every project brief, and the last scrim) with a Pro account in September 2026. This is what is actually inside.

*This page is part of our [Scrimba CSS courses](/docs/courses/css/) catalog.*

## Quick answer

Learn UI Design is a Pro-only, intermediate course: 8.6 hours across twelve modules, 100 scrims plus a certificate outro, taught by Gary Simon. It is not a Figma course. Every lesson happens in Scrimba's editor: Gary shows you a bad card, form, or dashboard, names which of his seven fundamentals it breaks (white space, alignment, contrast, scale, typography, color, visual hierarchy), and has you fix it in CSS before he shows his own fix. Roughly 45 of the scrims are challenges of that kind.

You need basic HTML and CSS to follow along and a little JavaScript for two lessons. If you can build a layout but your work never looks finished, this is the most direct course on Scrimba for that problem.
...
Two caveats. First, it is a design course that assumes your CSS is solid. Gary types fast, pastes in blocks of rules, and expects you to know what `grid-template-areas`, `position: absolute`, and `::before` do. If those are shaky, do [Learn HTML and CSS](/docs/courses/css/html-and-css/) first.

Second, the recordings are a few years old. Gary refers to Adobe XD alongside Figma, the JavaScript is old-school `addEventListener` code, and a couple of the tool links he reads out (a box-shadow generator, a layered-shadow generator) may have moved. None of the design advice has aged, because none of it depends on a tool.
