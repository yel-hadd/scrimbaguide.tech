...
Scrimba's CSS practice pack, taught by Treasure Porth: 25 challenges, each with a brief and a recorded solution, in about 2.6 hours of video. I opened 43 of its 54 scrims with a Pro account in September 2026 (every challenge brief, most of the solutions, the intro and the outro) and read their transcripts. This is what the course asks of you, what it hands you, and who it suits.
...
CSS Challenges is a Pro, intermediate-level practice course: 54 short scrims, about 2.6 hours of video, taught by Treasure Porth. There are no lessons in the usual sense. Each of the 25 challenges is a one to three minute brief (an animated example, a requirements slide, some hints) followed by a three to nine minute solution where Treasure builds it. The targets range from a gradient button and GitHub's expanding search bar to seven country flags, a toggle switch, Instagram's stories menu, and GitHub's contributions graph. The first eleven scrims (through Loading Animation 1) are free samples; the remaining 43 need Pro. If you already know flexbox, grid and positioning and want to find out how well you can apply them, this is a good two-week workout. If you are still learning those, it will frustrate you.
...
<div className="screenshot-grid">
  <Screenshot
    ...
    width={1280}
    height={684}
    caption="The welcome scrim's title slide. The course has no chapters, so this is the only title card."
    source=""
  />
  <Screenshot
    src="/img/scrimba/css-challenges/colorful-button-requirements-slide.webp"
    alt="Requirements slide from the Colorful Button challenge in Scrimba's CSS Challenges: gradient border at 115 degrees, hover state where the gradient flips horizontally, the button grows and text turns from gray to white, and center the button on the page"
    width={1280}
    height={720}
    caption="Every challenge brief has a slide like this one (Colorful Button, 0:56). The requirements are short and testable."
...
<Screenshot
  ...
  alt="Scrimba CSS Challenges, Solution: Loading Animation 1 paused at 2:43: index.css shows a .circle rule with animation: pulse infinite alternate 1s and a @keyframes pulse block going from a dark background to a light one at 1.6em, while the preview shows three gray dots centered in a white pane"
  width={1400}
  height={660}
...
<Screenshot
  ...
  alt="Scrimba CSS Challenges, Solution: Codepen Tile paused at 5:44: index.css shows the :root variables for the Noto Sans font and the card, stats and username colors, plus a body rule that applies them; the preview shows the finished tile with a purple gradient thumbnail, the title Abstract Gradient Project, a cat avatar and the username Scrimsbee McScrimslee"
  width={1400}
  height={660}
...
2.6 hours is the video. The video is mostly Treasure's solutions, and the whole point is to attempt each challenge before watching hers, so budget 20 to 40 minutes of your own work per challenge for the harder ones (Sweden, the toggle, the flashcard, the Instagram menu) and five to ten for the easy flags. That puts the honest total at six to nine hours: two weeks at half an hour a day, or two long weekends. If you only watch the solutions, it is an afternoon, and you will retain little of it.
