...
- Since 2026-09-26 monetised links send the full page URL as the referrer (`referrerPolicy=no-referrer-when-downgrade`, owner decision), so new Scrimbassadors Visitors/Subscribers rows can show the scrimbaguide.tech page in the Referrer column; rows before that date show none. When reading Visitors or Subscribers, count referrers that start with `https://scrimbaguide.tech/` per page. GA4 `affiliate_link_clicked` stays the main per-page source (optionally joined by date on Scrimba landing page = our `destination_slug`).
